...
Panel wiring from 2026-09-23 (D00 T04 §27): every round runs a slot from `.conclave/panel.toml`, GPT on every governing slot (`bulk` medium for Full rounds 1-2, `signoff` and `depth` high after), the writer's family only as the double-outage `cross-fill`; round headings read `GPT panel` or `Claude panel`, and a round's `model:` must be a registered model of its heading's family. Blocks before that date keep the wiring described next.

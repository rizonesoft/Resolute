...
- **One writer, and it never reviews itself:** Claude Code is the only writer (operator decision 2026-09-23); Muse, Grok, and other harnesses do not edit, commit, or run campaigns here. Every other model takes part only as a headless reviewer through a slot in `.conclave/panel.toml`, which names the writer, registers the models, and pins every review role; `scripts/panel_slots.py` refuses a table where any slot shares the writer's family. GPT reviews and the newest Grok model is every fallback (D00 T04 §29), resolved at run time so a new Grok release needs no edit. Skills name slots, never models: a re-pin is one edit to that file plus a fresh probe date.

...
- Cleaner Icon Hover functions.
- New Global error count for the logging subsystem.
- Moved Documents to Documents folder.
- New Help (chm) file.
- 7zip Prerequisite now bundled with SDK.
- Improved Prerequisite detection.
- Finished the Refresh Prerequisites button.
- Unused Prerequisite rows are now Hidden.
- Prerequisite information tooltips.
- Disable related Options if Prerequisite is unavailable.
- Added Tidy Source Code option.
- Distros now called Solutions.
- Removed Checkboxes from Solutions List. Only work on one release at a time.
- New Modules Section. Disable unsupported options on Solution selection.
- Mouse over event now not firing on disabled Options.
- Now remembers the last selected Solution.
- New Icon an minor cosmetic changes.

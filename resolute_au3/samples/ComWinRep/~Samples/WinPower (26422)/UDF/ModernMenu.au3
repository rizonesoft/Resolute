;********************************************************************
; ModernMenu UDF by Derick Payne
; Version 2009/09/27
;********************************************************************

#include-once

Global $sLOGFONT = "int;" & "int;" & "int;" & "int;" & "int;" & "byte;" & "byte;" & "byte;" & "byte;" & _
				   "byte;" & "byte;" & "byte;" & "byte;" & "wchar[32]"
Global Const $sNONCLIENTMETRICS = "uint;" & "int;" & "int;" & "int;" & "int;" & "int;" & _
                                  $sLOGFONT & ";" & "int;" & "int;" & $sLOGFONT & ";" & "int;" & _
								  "int;" & $sLOGFONT & ";" & $sLOGFONT & ";" & $sLOGFONT
Global Const $sMENUITEMINFO = "uint;" & "uint;" & "uint;" & "uint;" & "uint;" & "hwnd;" & "hwnd;" & "hwnd;" & "long;" & "ptr;" & "uint;" & "hwnd"

;**********************************************************************
; NotifyIconData struct
;**********************************************************************
Global $sNOTIFYICONDATAW = "dword;" & "hwnd;" & "uint;" & "uint;" & "uint;" & "hwnd;" & "wchar[128];" & "dword;" & "dword;" & "wchar[256];" & "uint;" & "wchar[64];" & "dword"
...
;~ ;If Not IsDeclared("WM_LBUTTONUP") Then Local Const $WM_LBUTTONUP = 0x0202
...
; Main Creation Part
;********************************************************************
Global $hComctl32Dll = DllOpen("comctl32.dll"), $hGdi32Dll = DllOpen("gdi32.dll"), $hKernel32Dll = DllOpen("kernel32.dll"), $hShell32Dll = DllOpen("shell32.dll"), $hUser32Dll = DllOpen("user32.dll"), $hMsimg32Dll = DllOpen("msimg32.dll")
Global $bUseAdvMenu = TRUE, $bUseAdvTrayMenu = TRUE, $bUseRGBColors = FALSE

; Set default color values if not given
Global $nMenuBkClr = 0xFFFFFF, $nMenuIconBkClr = 0xDBD8D8, $nMenuSelectBkClr = 0xD2BDB6, $nMenuSelectRectClr = 0x854240, $nMenuSelectTextClr = 0x000000, $nMenuTextClr = 0x000000, $nMenuSideBkClr = 0xD00000, $nMenuSideTxtClr = 0xFFFFFF
Global $nTrayBkClr = 0xFFFFFF, $nTrayIconBkClr = 0xD1D8DB, $nTraySelectBkClr = 0xD2BDB6, $nTraySelectRectClr = 0x854240, $nTraySelectTextClr = 0x000000, $nTrayTextClr = 0x000000
...
Global $nMenuIconBkClr2 = $nMenuIconBkClr, $nMenuSideBkClr2 = $nMenuSideBkClr, $nTrayIconBkClr2 = $nTrayIconBkClr
...
$TRAYNOTIFYIDS[0][0] = 0
Global $TRAYMSGWND = 0, $TRAYNOTIFYID = -1, $TRAYLASTID = -1, $MENULASTITEM = -1, $TRAYLASTITEM = -1, $TRAYLASTMENU = -1, $TRAYTIPMSG = 0x0400 + 1, $FLASHTIMERID = 3, $FLASHTIMEOUT = 750, $sDefaultTT = "AutoIt - " & @ScriptName
...
Func OnAutoItExit()
	...
	For $i = 1 To $arSideItems[0][0]
		If $arSideItems[$i][6] <> 0 Then DeleteObject($arSideItems[$i][6])
	Next
...
Func _TrayNotifyIcon($hWnd, $Msg, $wParam, $lParam)
	...
	If $TRAYNOTIFYIDS[$nID][5] <> "" And ($TRAYNOTIFYIDS[$nID][6] = 0 Or $TRAYNOTIFYIDS[$nID][6] = $lParam) Then
...
Func _GetNewTrayIndex()
	...
	If Not $bFreeFound Then
		...
		If $TRAYNOTIFYIDS[0][0] > $nSize - 10 Then Redim $TRAYNOTIFYIDS[$nSize + 10][9]
		$i = $TRAYNOTIFYIDS[0][0]
	EndIf
...
Func _CreateBlankIcon()
	If $hBlankIcon = 0 Then
		If @OSVersion = "WIN_VISTA" Or @OSVersion = "WIN_2008" Then
			Local $stIcon = DllStructCreate("hwnd")
			If ExtractIconExW("shell32.dll", 50, 0, DllStructGetPtr($stIcon), 1) Then $hBlankIcon = DllStructGetData($stIcon, 1)
		Else
...
Func MulDiv($nInt1, $nInt2, $nInt3)
	Local $nResult = 0
	$nResult = DllCall("kernel32.dll", "int", "MulDiv", "int", $nInt1, "int", $nInt2, "int", $nInt3)
	Return $nResult[0]
EndFunc

Func LoadLibraryExW($pFile, $hFile, $nFlags)
	Local $hResult = DllCall($hKernel32Dll, "hwnd", "LoadLibraryExW", "ptr", $pFile, "hwnd", $hFile, "dword", $nFlags)
	Return $hResult[0]
EndFunc

Func FreeLibrary($hLib)
	Local $nResult = DllCall($hKernel32Dll, "int", "FreeLibrary", "hwnd", $hLib)
	Return $nResult[0]
EndFunc
...
Func SelectObject($hDC, $hObj)
	Local $hOldObj = DllCall($hGdi32Dll, "int", "SelectObject", "hwnd", $hDC, "hwnd", $hObj)
	Return $hOldObj[0]
EndFunc

Func DeleteObject($hObj)
	Local $bResult = DllCall($hGdi32Dll, "int", "DeleteObject", "hwnd", $hObj)
	Return $bResult[0]
EndFunc

Func GetObjectW($hObj, $nSize, $pObj)
	Local $nResult = DllCall($hGdi32Dll, "int", "GetObjectW", "hwnd", $hObj, "int", $nSize, "ptr", $pObj)
	Return $nResult[0]
EndFunc
...
Func CreateFont($nHeight, $nWidth, $nEscape, $nOrientn, $fnWeight, $bItalic, $bUnderline, $bStrikeout, $nCharset, $nOutputPrec, $nClipPrec, $nQuality, $nPitch, $ptrFontName)
	Local $hFont = DllCall($hGdi32Dll, "hwnd", "CreateFont", "int", $nHeight, "int", $nWidth, "int", $nEscape, "int", $nOrientn, "int", $fnWeight, "long", $bItalic, "long", $bUnderline, "long", $bStrikeout, "long", $nCharset, "long", $nOutputPrec, "long", $nClipPrec, "long", $nQuality, "long", $nPitch, "ptr", $ptrFontName)
	Return $hFont[0]
EndFunc

Func CreateFontIndirect($pLogFont)
	Local $hFont = DllCall($hGdi32Dll, "hwnd", "CreateFontIndirectW", "ptr", $pLogFont)
	Return $hFont[0]
EndFunc
...
Func SetBkColor($hDC, $nColor)
	Local $nOldColor = DllCall($hGdi32Dll, "int", "SetBkColor", "hwnd", $hDC, "int", $nColor)
	Return $nOldColor[0]
EndFunc

Func SetTextColor($hDC, $nColor)
	Local $nOldColor = DllCall($hGdi32Dll, "int", "SetTextColor", "hwnd", $hDC, "int", $nColor)
	Return $nOldColor[0]
EndFunc
...
Func GetDeviceCaps($hDC, $nIndex)
	Local $nResult = DllCall($hGdi32Dll, "int", "GetDeviceCaps", "hwnd", $hDC, "int", $nIndex)
	Return $nResult[0]
EndFunc

Func CreateCompatibleDC($hDC)
	Local $hCompDC = DllCall($hGdi32Dll, "hwnd", "CreateCompatibleDC", "hwnd", $hDC)
	Return $hCompDC[0]
EndFunc

Func DeleteDC($hDC)
	Local $bResult = DllCall($hGdi32Dll, "int", "DeleteDC", "hwnd", $hDC)
	Return $bResult[0]
EndFunc

Func CreateBitmap($nWidth, $nHeight, $nCPlanes, $nCBitsPerPixel, $ptrCData)
	Local $hBitmap = DllCall($hGdi32Dll, "hwnd", "CreateBitmap", "int", $nWidth, "int", $nHeight, "int", $nCPlanes, "int", $nCBitsPerPixel, "ptr", $ptrCData)
	Return $hBitmap[0]
EndFunc

Func BitBlt($hDCDest, $nXDest, $nYDest, $nWDest, $nHDest, $hDCSrc, $nXSrc, $nYSrc, $nOpCode)
	Local $bResult = DllCall($hGdi32Dll, "int", "BitBlt", "hwnd", $hDCDest, "int", $nXDest, "int", $nYDest, "int", $nWDest, "int", $nHDest, "hwnd", $hDCSrc, "int", $nXSrc, "int", $nYSrc, "long", $nOpCode)
	Return $bResult[0]
EndFunc

Func StretchBlt($hDCDest, $nXDest, $nYDest, $nWDest, $nHDest, $hDCSrc, $nXSrc, $nYSrc, $nWSrc, $nHSrc, $nOpCode)
	Local $bResult = DllCall($hGdi32Dll, "int", "StretchBlt", "hwnd", $hDCDest, "int", $nXDest, "int", $nYDest, "int", $nWDest, "int", $nHDest, "hwnd", $hDCSrc, "int", $nXSrc, "int", $nYSrc, "int", $nWSrc, "int", $nHSrc, "long", $nOpCode)
	Return $bResult[0]
EndFunc

Func SetBkMode($hDC, $nMode)
	Local $nResult = DllCall($hGdi32Dll, "int", "SetBkMode", "hwnd", $hDC, "int", $nMode)
	Return $nResult[0]
EndFunc

Func GradientFill($hDC, $pVert, $nNumVert, $pRect, $nNumRect, $nFillMode)
	Local $nResult = DllCall($hMsimg32Dll, "int", "GradientFill", "hwnd", $hDC, "ptr", $pVert, "ulong", $nNumVert, "ptr", $pRect, "ulong", $nNumRect, "ulong", $nFillMode)
	Return $nResult[0]
EndFunc
...
;********************************************************************
Func GetDC($hWnd)
	Local $hDC = DllCall($hUser32Dll, "hwnd", "GetDC", "hwnd", $hWnd)
	Return $hDC[0]
EndFunc

Func ReleaseDC($hWnd, $hDC)
	Local $bResult = DllCall($hUser32Dll, "int", "ReleaseDC", "hwnd", $hWnd, "hwnd", $hDC)
	Return $bResult[0]
EndFunc

Func GetSysColor($nIndex)
	Local $nColor = DllCall($hUser32Dll, "int", "GetSysColor", "int", $nIndex)
	Return $nColor[0]
EndFunc

Func GetSysColorBrush($nIndex)
	Local $hBrush = DllCall($hUser32Dll, "hwnd", "GetSysColorBrush", "int", $nIndex)
	Return $hBrush[0]
EndFunc

Func LoadIcon($hInstance, $nIcon)
	Local $hIcon = DllCall($hUser32Dll, "hwnd", "LoadIcon", "hwnd", $hInstance, "int", $nIcon)
	Return $hIcon[0]
EndFunc

Func DestroyIcon($hIcon)
	Local $bResult = DllCall($hUser32Dll, "int", "DestroyIcon", "hwnd", $hIcon)
	Return $bResult[0]
EndFunc

Func CreateIcon($hInstance, $nWidth, $nHeight, $nPlanes, $nBitsPixel, $pAndBits, $pXorBits)
	Local $hResult = DllCall($hUser32Dll, "hwnd", "CreateIcon", "hwnd", $hInstance, "int", $nWidth, "int", $nHeight, "byte", $nPlanes, "byte", $nBitsPixel, "ptr", $pAndBits, "ptr", $pXorBits)
	Return $hResult[0]
EndFunc

Func GetSystemMetrics($nIndex)
	Local $nResult = DllCall($hUser32Dll, "int", "GetSystemMetrics", "int", $nIndex)
	Return $nResult[0]
EndFunc
...
Func FillRect($hDC, $ptrRect, $hBrush)
	Local $bResult = DllCall($hUser32Dll, "int", "FillRect", "hwnd", $hDC, "ptr", $ptrRect, "hwnd", $hBrush)
	Return $bResult[0]
EndFunc

Func DrawEdge($hDC, $ptrRect, $nEdgeType, $nBorderFlag)
	Local $bResult = DllCall($hUser32Dll, "int", "DrawEdge", "hwnd", $hDC, "ptr", $ptrRect, "int", $nEdgeType, "int", $nBorderFlag)
	Return $bResult[0]
EndFunc

Func FrameRect($hDC, $ptrRect, $hBrush)
	Local $bResult = DllCall($hUser32Dll, "int", "FrameRect", "hwnd", $hDC, "ptr", $ptrRect, "hwnd", $hBrush)
	Return $bResult[0]
EndFunc

Func DrawFrameControl($hDC, $ptrRect, $nType, $nState)
	Local $bResult = DllCall($hUser32Dll, "int", "DrawFrameControl", "hwnd", $hDC, "ptr", $ptrRect, "int", $nType, "int", $nState)
	Return $bResult[0]
EndFunc

Func SystemParametersInfo($nAction, $nParam, $pParam, $nWinini)
	Local $nResult = DllCall($hUser32Dll, "int", "SystemParametersInfoW", "uint", $nAction, "uint", $nParam, "ptr", $pParam, "uint", $nWinini)
	Return $nResult[0]
EndFunc

Func GetCursorPos($pPoint)
	Local $nResult = DllCall($hUser32Dll, "int", "GetCursorPos", "ptr", $pPoint)
	Return $nResult[0]
EndFunc

Func SetForegroundWindow($hWnd)
	Local $nResult = DllCall($hUser32Dll, "int", "SetForegroundWindow", "hwnd", $hWnd)
	Return $nResult[0]
EndFunc

Func TrackPopupMenuEx($hMenu, $nFlags, $nX, $nY, $hWnd, $pParams)
    Local $nResult = DllCall($hUser32Dll, "int", "TrackPopupMenuEx", "hwnd", $hMenu, "uint", $nFlags, "int", $nX, "int", $nY, "hwnd", $hWnd, "ptr", $pParams)
    Return $nResult[0]
EndFunc

Func PostMessage($hWnd, $nMsg, $wParam, $lParam)
    Local $nResult = DllCall($hUser32Dll, "int", "PostMessage", "hwnd", $hWnd, "uint", $nMsg, "dword", $wParam, "dword", $lParam)
    Return $nResult[0]
EndFunc

Func ShowWindow($hWnd, $nState)
	DllCall($hUser32Dll, "int", "ShowWindow", "hwnd", $hWnd, "int", $nState)
EndFunc

Func SetTimer($hWnd, $nID, $nTimeOut, $pFunc)
	Local $nResult = DllCall($hUser32Dll, "uint", "SetTimer", "hwnd", $hWnd, "uint", $nID, "uint", $nTimeOut, "ptr", $pFunc)
	Return $nResult[0]
EndFunc

Func KillTimer($hWnd, $nID)
	Local $nResult = DllCall($hUser32Dll, "int", "KillTimer", "hwnd", $hWnd, "uint", $nID)
	Return $nResult[0]
EndFunc

Func LoadImageW($hInst, $pName, $nType, $nX, $nY, $nLoad)
	Local $hResult = DllCall($hUser32Dll, "hwnd", "LoadImageW", "hwnd", $hInst, "dword", $pName, "uint", $nType, "int", $nX, "int", $nY, "uint", $nLoad)
	Return $hResult[0]
EndFunc

Func memset($pDest, $nChr, $nCount)
	Local $pResult = DllCall("msvcrt.dll", "ptr:cdecl", "memset", "ptr", $pDest, "int", $nChr, "int", $nCount)
	Return $pResult[0]
EndFunc

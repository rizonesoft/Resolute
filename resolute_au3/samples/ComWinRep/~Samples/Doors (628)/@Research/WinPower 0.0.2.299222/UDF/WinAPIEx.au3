...
Global Const $tagBITMAP = 'long Type;long Width;long Height;long WidthBytes;ushort Planes;ushort BitsPixel;ptr Bits'
Global Const $tagBITMAPINFOHEADER = 'dword Size;long Width;long Height;ushort Planes;ushort BitCount;dword Compression;dword SizeImage;long XPelsPerMeter;long YPelsPerMeter;dword ClrUsed;dword ClrImportant'
Global Const $tagDIBSECTION = $tagBITMAP & ';' & $tagBITMAPINFOHEADER & ';dword Bitfields[3];ptr hSection;dword Offset'
Global Const $tagDISKGEOMETRY = 'int64 Cylinders;dword MediaType;dword TracksPerCylinder;dword SectorsPerTrack;dword BytesPerSector'
Global Const $tagDISKGEOMETRYEX = $tagDISKGEOMETRY & ';int64 DiskSize;dword Data'
...
Global Const $tagSHFILEINFO = 'ptr hIcon;int iIcon;dword Attributes;char DisplayName[1024];char TypeName[80]'
Global Const $tagOSVERSIONINFO = 'dword OSVersionInfoSize;dword MajorVersion;dword MinorVersion;dword BuildNumber;dword PlatformId;char CSDVersion[128]'
...
Global Const $tagPROCESSMEMORYCOUNTERS = 'dword Size;dword PageFaultCount;long PeakWorkingSetSize;long WorkingSetSize;long QuotaPeakPagedPoolUsage;long QuotaPagedPoolUsage;long QuotaPeakNonPagedPoolUsage;long QuotaNonPagedPoolUsage;long PagefileUsage;long PeakPagefileUsage'
Global Const $tagSTORAGEDEVICENUMBER = 'dword DeviceType;ulong DeviceNumber;ulong PartitionNumber'
...
;                  |TRUE  - Use the Alpha channel (that is, per-pixel alpha). Note that the APIs use premultiplied alpha, which
;                           means that the red, green and blue channel values in the bitmap must be premultiplied with the alpha
;                           channel value. For example, if the alpha channel value is x, the red, green and blue channels must
;                           be multiplied by x and divided by 255 prior to the call.
...
;                  $iFlags    - Specifies the type of animation. This parameter can be one or more of the following values. Note that,
;                               by default, these flags take effect when showing a window. To take effect when hiding a window, use AW_HIDE and
;                               a logical OR operator with the appropriate flags.
;
;                               $AW_SLIDE
;                               $AW_ACTIVATE
;                               $AW_BLEND
...
Func _WinAPI_BeginPaint($hWnd, ByRef $tPAINTSTRUCT)
	...
	If (@error) Or ($Ret[0] = 0) Then
		$tPAINTSTRUCT = 0
	EndIf
	Return SetError(Int(IsDllStruct($tPAINTSTRUCT)), 0, $Ret[0])
...
Func _WinAPI_CopyFileEx($sSource, $sDestination, $pProgressRoutine, $iFlags = 0)
	...
	Local $Ret = DllCall('kernel32.dll', 'int', 'CopyFileEx', 'str', $sSource, 'str', $sDestination, 'ptr', $pProgressRoutine, 'ptr', 0, 'int', 0, 'int', $iFlags)
...
Func _WinAPI_CreateBrushIndirect($iStyle, $iRGB, $iHatch = 0)
	...
	DllStructSetData($tLOGBRUSH, 2, __CheckColor($iRGB))
...
Func _WinAPI_CreateDIBSection($hDC, $tBITMAPINFO, $iUsage, ByRef $pBits, $hSection = 0, $iOffset = 0)
	...
	Else
		$pBits = $Ret[4]
	EndIf
	Return SetError(Int($Ret[0] = 0), 0, $Ret[0])
...
Func _WinAPI_DrawBitmap($hDC, $iX, $iY, $hBitmap)
	...
	$hSrcSv = $Ret[0]
	$Ret = DllCall('gdi32.dll', 'int', 'BitBlt', 'hwnd', $hDC, 'int', $iX, 'int', $iY, 'int', DllStructGetData($tObj, 'Width'), 'int', DllStructGetData($tObj, 'Height'), 'hwnd', $hSrcDC, 'int', 0, 'int', 0, 'int', 0x00CC0020)
	If (@error) Or ($Ret[0] = 0) Then
		$Ret = 0
	EndIf
...
Func _WinAPI_DrawShadowText($hDC, $sText, $rgbText, $rgbShadow, $iXOffset = 0, $iYOffset = 0, $tRECT = 0, $iFlags = 0)
	...
	If Not IsDllStruct($tRECT) Then
		...
		EndIf
	EndIf
	$Ret = DllCall('comctl32.dll', 'int', 'DrawShadowText', 'hwnd', $hDC, 'wstr', $sText, 'uint', -1, 'ptr', DllStructGetPtr($tRECT), 'dword', $iFlags, 'int', __CheckColor($rgbText), 'int', __CheckColor($rgbShadow), 'int', $iXOffset, 'int', $iYOffset)
	If (@error) Or ($Ret[0] = 0) Then
		Return SetError(1, 0, 0)
	EndIf
	Return 1
...
;                  $fForce - Specifies whether enumerates the invisible window, valid values:
...
Func _WinAPI_EnumChildWindows($hWnd, $fVisible = 1)
	...
	Local $hEnumProc, $Error = 1

	Dim $__Data[101][2] = [[0]]
	$hEnumProc = DllCallbackRegister('__EnumChildWindowsProc','int','hwnd;int')
...
Func _WinAPI_EnumDisplaySettings($sDevice, $iMode)
	...
	Local $tDEVMODE = DllStructCreate('char[32];short;short;short;short;dword;dword[2];dword;dword;short;short;short;short;short;char[32];short;dword;dword;dword;dword;dword')

	DllStructSetData($tDEVMODE, 4, DllStructGetSize($tDEVMODE))
	DllStructSetData($tDEVMODE, 5, 0)

	Local $Ret = DllCall('user32.dll', 'int', 'EnumDisplaySettings', $TypeOfDevice, $sDevice, 'dword', $iMode, 'ptr', DllStructGetPtr($tDEVMODE))
...
Func _WinAPI_EnumResourceLanguages($sModule, $sType, $sName)
	...
	Local $Ret, $Result = 0, $hModule, $hEnumProc, $TypeOfName = 'long', $TypeOfType = 'long'
	...
	EndIf
	_WinAPI_SetLastError($Result)
	Return SetError(Int($__Data = 0), 0, $__Data)
EndFunc   ;==>_WinAPI_EnumResourceLanguages
...
Func _WinAPI_EnumResourceNames($sModule, $sType)
	...
	Local $Ret, $Result = 0, $hModule, $hEnumProc, $TypeOfType = 'long'
...
Func _WinAPI_EnumResourceTypes($sModule)
	...
	Local $Ret, $Result = 0, $hModule, $hEnumProc
...
; Return values..: Success    - $tagSYSTEMTIME structure that contains the the converted file time.
;                  Failure    - 0 and sets the @error flag to non-zero.
; Author.........: Yashied
; Modified.......:
; Remarks........: None
...
Func _WinAPI_FitToBitmap($hBitmap, $iWidth, $iHeight, $iMode = 3)
	...
	$Ret = _WinAPI_StretchBlt($hDestDC, 0, 0, $iWidth, $iHeight, $hSrcDC, 0, 0, DllStructGetData($tObj, 'Width'), DllStructGetData($tObj, 'Height'), 0x00CC0020)
...
Func _WinAPI_GetBValue($iRGB)
	Return BitAND(__CheckColor($iRGB), 0x0000FF)
...
Func _WinAPI_GetCompression($sPath)
	...
	Local $hFile = DllCall('kernel32.dll', 'hwnd', 'CreateFile', 'str', $sPath, 'int', 0x80000000, 'int', 0x00000001, 'ptr', 0, 'int', 3, 'int', 0x02000000, 'ptr', 0)
...
Func _WinAPI_GetDriveBusType($sDrive)
	...
	Local $tSTORAGEPROPERTYQUERY = DllStructCreate('dword;dword;byte[4]')
	Local $tSTORAGEDEVICEDESCRIPTOR = DllStructCreate('ulong;ulong;byte;byte;byte;byte;ulong;ulong;ulong;ulong;dword;ulong;byte[512]')
	Local $Ret = DllCall('kernel32.dll', 'int', 'DeviceIoControl', 'ptr', $hFile, 'dword', 0x002D1400, 'ptr', DllStructGetPtr($tSTORAGEPROPERTYQUERY), 'dword', DllStructGetSize($tSTORAGEPROPERTYQUERY), 'ptr', DllStructGetPtr($tSTORAGEDEVICEDESCRIPTOR), 'dword', DllStructGetSize($tSTORAGEDEVICEDESCRIPTOR), 'dword*', 0, 'ptr', 0)
	...
	EndIf
	Return SetError(0, 0, DllStructGetData($tSTORAGEDEVICEDESCRIPTOR, 11))
...
; Return values..: Success - $tagDISKGEOMETRYEX structure that contains the information about the disk's geometry.
;                  Failure - 0 and sets the @error flag to non-zero.
; Author.........: Yashied
; Modified.......:
; Remarks........: None
...
; Parameters.....: $sDrive - The drive letter to retrieve information, in the format D:, E:, etc.
; Return values..: Success - $tagSTORAGEDEVICENUMBER structure that contains the relevant information.
;                  Failure - 0 and sets the @error flag to non-zero.
; Author.........: Yashied
; Modified.......:
; Remarks........: None
...
Func _WinAPI_GetDriveNumber($sDrive)
	...
	Local $tSTORAGEDEVICENUMBER = DllStructCreate($tagSTORAGEDEVICENUMBER)
	Local $Ret = DllCall('kernel32.dll', 'int', 'DeviceIoControl', 'ptr', $hFile, 'dword', 0x002D1080, 'ptr', 0, 'dword', 0, 'ptr', DllStructGetPtr($tSTORAGEDEVICENUMBER), 'dword', DllStructGetSize($tSTORAGEDEVICENUMBER), 'dword*', 0, 'ptr', 0)
	...
	If Not IsArray($Ret) Then
		Return SetError(2, 0, 0)
	EndIf
	Return SetError(0, 0, $tSTORAGEDEVICENUMBER)
...
Func _WinAPI_GetGValue($iRGB)
	Return BitShift(BitAND(__CheckColor($iRGB), 0x00FF00), 8)
...
; Syntax.........: _WinAPI_GetLocaleInfo ( $iLocale, $iType )
; Parameters.....: $iLocale - Locale identifier for which to retrieve information.
...
; Parameters.....: $PID    - The PID of the process. Default (0) is the current process.
; Return values..: Success - $tagSYSTEMTIME structure that contains the creation time of the process.
;                  Failure - 0 and sets the @error flag to non-zero.
; Author.........: Yashied
; Modified.......:
; Remarks........: None
...
Func _WinAPI_GetProcessCreationTime($PID = 0)
	...
	Local $tTime = DllStructCreate('dword;dword')
	Local $Ret = DllCall('kernel32.dll', 'int', 'GetProcessTimes', 'ptr', $hProc, 'ptr', DllStructGetPtr($tTime), 'ptr*', 0, 'ptr*', 0, 'ptr*', 0)
	...
	If Not IsArray($Ret) Then
		Return SetError(1, 0, 0)
	EndIf
	Return _WinAPI_FileTimeToSystemTime(_WinAPI_FileTimeToLocalFileTime($tTime))
...
; Parameters.....: $PID    - The PID of the process. Default (0) is the current process.
; Return values..: Success - $tagPROCESSMEMORYCOUNTERSEX structure that contains the information about the memory usage.
;                  Failure - 0 and sets the @error flag to non-zero.
; Author.........: Yashied
; Modified.......:
; Remarks........: None
...
Func _WinAPI_GetProcessMemoryInfo($PID = 0)
	...
	Local $tPROCESSMEMORYCOUNTERS = DllStructCreate($tagPROCESSMEMORYCOUNTERS)
	Local $Ret = DllCall('psapi.dll', 'int', 'GetProcessMemoryInfo', 'ptr', $hProc, 'ptr', DllStructGetPtr($tPROCESSMEMORYCOUNTERS), 'int', DllStructGetSize($tPROCESSMEMORYCOUNTERS))
	...
	If Not IsArray($Ret) Then
		Return SetError(1, 0, 0)
	EndIf
	Return SetError(0, 0, $tPROCESSMEMORYCOUNTERS)
...
Func _WinAPI_GetRValue($iRGB)
	Return BitShift(BitAND(__CheckColor($iRGB), 0xFF0000), 16)
...
Func _WinAPI_GetSystemPowerStatus()
	...
	Local $tSYSTEMPOWERSTATUS = DllStructCreate('byte;byte;byte;byte;dword;dword')
	Local $Ret = DllCall('kernel32.dll', 'int', 'GetSystemPowerStatus', 'ptr', DllStructGetPtr($tSYSTEMPOWERSTATUS))
	...
	$Result[0] = DllStructGetData($tSYSTEMPOWERSTATUS, 1)
	$Result[1] = DllStructGetData($tSYSTEMPOWERSTATUS, 2)
	$Result[2] = DllStructGetData($tSYSTEMPOWERSTATUS, 3)
	$Result[3] = DllStructGetData($tSYSTEMPOWERSTATUS, 5)
	$Result[4] = DllStructGetData($tSYSTEMPOWERSTATUS, 6)
...
Func _WinAPI_GetThemeColor($hWnd, $sClass, $iPart, $iState, $iProp)
	...
	If $Ret = -1 Then
		Return SetError(1, 0, -1)
	EndIf
	Return SetError(0, 0, __CheckColor($Ret[5]))
...
Func _WinAPI_GetVersionEx()
	...
	Local $Ret = DllCall('kernel32.dll', 'int', 'GetVersionEx', 'ptr', DllStructGetPtr($tOSVERSIONINFOEX))
...
; #FUNCTION# ====================================================================================================================
; Name...........: _WinAPI_GlobalSize
; Description....: Retrieves the current size of the specified global memory object.
; Syntax.........: _WinAPI_GlobalSize ( $hMem )
; Parameters.....: $hMem   - Handle to the global memory object.
; Return values..: Success - The size of the specified global memory object, in bytes.
;                  Failure - 0 and sets the @error flag to non-zero.
; Author.........: Yashied
; Modified.......:
; Remarks........: The size of a memory block may be larger than the size requested when the memory was allocated.
; Related........:
; Link...........: @@MsdnLink@@ GlobalSize
; Example........: Yes
; ===============================================================================================================================

Func _WinAPI_GlobalSize($hMem)

    Local $Ret = DllCall('kernel32.dll', 'int', 'GlobalSize', 'ptr', $hMem)

	If (@error) Or ($Ret[0] = 0) Then
		Return SetError(1, 0, 0)
	EndIf
	Return $Ret[0]
EndFunc   ;==>_WinAPI_GlobalSize
...
Func _WinAPI_GradientFill($hDC, $aVertex, $iStart = 0, $iEnd = -1, $fRotate = 0)
	...
	For $i = $iStart To $iEnd
		...
		DllStructSetData($tVertex, $Count, BitShift(_WinAPI_GetBValue($aVertex[$i][2]), -8), 5)
		DllStructSetData($tVertex, $Count, BitShift(_WinAPI_GetGValue($aVertex[$i][2]), -8), 6)
		DllStructSetData($tVertex, $Count, BitShift(_WinAPI_GetRValue($aVertex[$i][2]), -8), 7)
...
Func _WinAPI_IsDoorOpen($sDrive)
	...
	Local $tSCSIPASSTHROUGH = DllStructCreate('ushort;ubyte;ubyte;ubyte;ubyte;ubyte;ubyte;ubyte;ubyte[3];uint;uint;uint;uint;ubyte[16];ubyte[240];ubyte[1024]')
	Local $pSCSIPASSTHROUGH = DllStructGetPtr($tSCSIPASSTHROUGH)

    DllStructSetData($tSCSIPASSTHROUGH, 1, 44)
    DllStructSetData($tSCSIPASSTHROUGH, 2, 0)
    DllStructSetData($tSCSIPASSTHROUGH, 3, 0)
    DllStructSetData($tSCSIPASSTHROUGH, 4, 0)
    DllStructSetData($tSCSIPASSTHROUGH, 5, 0)
    DllStructSetData($tSCSIPASSTHROUGH, 6, 12)
    DllStructSetData($tSCSIPASSTHROUGH, 7, 16)
    DllStructSetData($tSCSIPASSTHROUGH, 8, 1)
;   DllStructSetData($tSCSIPASSTHROUGH, 9, 0)
    DllStructSetData($tSCSIPASSTHROUGH, 10, 1024)
    DllStructSetData($tSCSIPASSTHROUGH, 11, 5)
    DllStructSetData($tSCSIPASSTHROUGH, 12, 44 + 240)
    DllStructSetData($tSCSIPASSTHROUGH, 13, 44)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 0xBD, 1)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 0, 2)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 0, 3)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 0, 4)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 0, 5)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 0, 6)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 0, 7)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 0, 8)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 0, 9)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 8, 10)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 0, 11)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 0, 12)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 0, 13)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 0, 14)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 0, 15)
    DllStructSetData($tSCSIPASSTHROUGH, 14, 0, 16)
    DllStructSetData($tSCSIPASSTHROUGH, 15, 0, 1)
    DllStructSetData($tSCSIPASSTHROUGH, 16, 0, 1)

	Local $Ret = DllCall('kernel32.dll', 'int', 'DeviceIoControl', 'ptr', $hFile, 'dword', 0x0004D004, 'ptr', $pSCSIPASSTHROUGH, 'dword', 44, 'ptr', $pSCSIPASSTHROUGH, 'dword', DllStructGetSize($tSCSIPASSTHROUGH), 'dword*', 0, 'ptr', 0)
	...
	If Not IsArray($Ret) Then
		Return SetError(2, 0, 0)
	EndIf
	Return SetError(0, 0, BitAND(DllStructGetData($tSCSIPASSTHROUGH, 16, 2), 0x10) = 0x10)
...
Func _WinAPI_LoadCursorFromFile($sFile)
	...
	Local $Ret = DllCall('user32.dll', 'ptr', 'LoadCursorFromFile', 'str', $sFile)
...
Func _WinAPI_LockDevice($sDrive, $fLock)
	...
	Local $tPREVENTMEDIAREMOVAL = DllStructCreate('byte')

	DllStructSetData($tPREVENTMEDIAREMOVAL, 1, $fLock)

	Local $Ret = DllCall('kernel32.dll', 'int', 'DeviceIoControl', 'ptr', $hFile, 'dword', 0x002D4804, 'ptr', DllStructGetPtr($tPREVENTMEDIAREMOVAL), 'dword', DllStructGetSize($tPREVENTMEDIAREMOVAL), 'ptr', 0, 'dword', 0, 'dword*', 0, 'ptr', 0)
...
Func _WinAPI_PrivateExtractIcon($sIcon, $iIndex, $iWidth, $iHeight)
	...
	Local $Ret = DllCall('user32.dll', 'int', 'PrivateExtractIcons', 'str', $sIcon, 'int', $iIndex, 'int', $iWidth, 'int', $iHeight, 'ptr*', 0, 'ptr*', 0, 'int', 1, 'int', 0)
...
Func _WinAPI_RGB($iRed, $iGreen, $iBlue)
	Return __CheckColor(BitOR(BitShift($iBlue, -16), BitShift($iGreen, -8), $iRed))
EndFunc   ;==>_WinAPI_RGB
...
Func _WinAPI_SetCompression($sPath, $iCompression)
	...
	Local $hFile = DllCall('kernel32.dll', 'hwnd', 'CreateFile', 'str', $sPath, 'int', 0xC0000000, 'int', 0x00000003, 'ptr', 0, 'int', 3, 'int', 0x02000000, 'ptr', 0)
...
Func _WinAPI_SetDCBrushColor($hDC, $iRGB)
	...
	Local $Ret = DllCall('gdi32.dll', 'dword', 'SetDCBrushColor', 'hwnd', $hDC, 'dword', __CheckColor($iRGB))
...
Func _WinAPI_SetDCPenColor($hDC, $iRGB)
	...
	Local $Ret = DllCall('gdi32.dll', 'dword', 'SetDCPenColor', 'hwnd', $hDC, 'dword', __CheckColor($iRGB))
...
Func _WinAPI_SetLayeredWindowAttributes($hWnd, $iRGB, $iAlpha, $iFlags)
	...
	Local $Ret = DllCall('user32.dll', 'int', 'SetLayeredWindowAttributes', 'hwnd', $hWnd, 'long', __CheckColor($iRGB), 'byte', $iAlpha, 'long', $iFlags)
...
Func _WinAPI_SetPixel($hDC, $iX, $iY, $iRGB)
	...
	Local $Ret = DllCall('gdi32.dll', 'int', 'SetPixelV', 'hwnd', $hDC, 'int', $iX, 'int', $iY, 'dword', __CheckColor($iRGB))
...
Func _WinAPI_ShellGetFileInfo($sPath, $iFlags, $iAttributes = 0x80)
	...
	Local $Ret = DllCall('shell32.dll', 'ptr', 'SHGetFileInfo', 'str', $sPath, 'dword', $iAttributes, 'ptr', DllStructGetPtr($tSHFILEINFO), 'int', DllStructGetSize($tSHFILEINFO), 'int', $iFlags)
...
Func _WinAPI_TransparentBlt($hDestDC, $iXDest, $iYDest, $iWidthDest, $iHeightDest, $hSrcDC, $iXSrc, $iYSrc, $iWidthSrc, $iHeightSrc, $iRGB)
	...
	Local $Ret  = DllCall('gdi32.dll', 'int', 'GdiTransparentBlt', 'hwnd', $hDestDC, 'int', $iXDest, 'int', $iYDest, 'int', $iWidthDest, 'uint', $iHeightDest, 'hwnd', $hSrcDC, 'int', $iXSrc, 'int', $iYSrc, 'int', $iWidthSrc, 'int', $iHeightSrc, 'dword', __CheckColor($iRGB))
...
Func __CheckColor($iColor)
	If $__RGB Then
		$iColor = _WinAPI_SwitchColor($iColor)
	EndIf
	Return $iColor
EndFunc   ;==>__CheckColor

Func __EnumChildWindowsProc($hWnd, $fVisible)
	...
	Return 1
EndFunc   ;==>__EnumChildWindowsProc
...
Func __EnumResNamesProc($hModule, $iType, $iName, $lParam)
	...
	Local $iSize = _WinAPI_GlobalSize($iName)
	Local $sName, $tName

	If $iSize > 0 Then
		$tName = DllStructCreate('char[' & $iSize & ']', $iName)
...
Func __EnumResTypesProc($hModule, $iType, $lParam)
	...
	Local $iSize = _WinAPI_GlobalSize($iType)
	Local $sType, $tType

	If $iSize > 0 Then
		$tType = DllStructCreate('char[' & $iSize & ']', $iType)

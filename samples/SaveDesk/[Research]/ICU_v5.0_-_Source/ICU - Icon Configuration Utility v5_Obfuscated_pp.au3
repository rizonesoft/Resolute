...
Local $3s = dllcall($0, "dword", "GetLastError")
Return SetError($3q, $3r, $3s[0])
EndFunc
Func _6($3t, $3u, $3v = 0, $3w = 0, $3x = 0, $3y = "wparam", $3z = "lparam", $40 = "lresult")
Local $3s = dllcall($1, $40, "SendMessageW", "hwnd", $3t, "uint", $3u, $3y, $3v, $3z, $3w)
...
Func _7($4b, $4c, $3v, $3w)
Local $3s = dllcall($1, "lresult", "CallNextHookEx", "handle", $4b, "int", $4c, "wparam", $3v, "lparam", $3w)
If @error Then Return SetError(@error, @extended, -1)
Return $3s[0]
EndFunc
Func _8($4d)
Local $3s = dllcall($3, "bool", "DeleteObject", "handle", $4d)
If @error Then Return SetError(@error, @extended, False)
Return $3s[0]
EndFunc
Func _9($3t)
If Not IsHWnd($3t) Then $3t = GUICtrlGetHandle($3t)
Local $3s = dllcall($1, "int", "GetClassNameW", "hwnd", $3t, "wstr", "", "int", 4096)
If @error Then Return SetError(@error, @extended, False)
Return SetExtended($3s[0], $3s[2])
EndFunc
Func _a()
Local $3s = dllcall($0, "handle", "GetCurrentThread")
...
$4f = "ptr"
EndIf
Local $3s = dllcall($0, "handle", "GetModuleHandleW", $4f, $4e)
If @error Then Return SetError(@error, @extended, 0)
Return $3s[0]
EndFunc
...
Local $3s = dllcall($1, "long_ptr", $4l, "hwnd", $3t, "int", $4k)
If @error Then Return SetError(@error, @extended, 0)
Return $3s[0]
EndFunc
Func _g($3t, ByRef $4m)
Local $3s = dllcall($1, "dword", "GetWindowThreadProcessId", "hwnd", $3t, "dword*", 0)
If @error Then Return SetError(@error, @extended, 0)
$4m = $3s[2]
Return $3s[0]
EndFunc
...
EndFunc
Func _k($3t, ByRef $4j)
Local $3s = dllcall($1, "bool", "ScreenToClient", "hwnd", $3t, "struct*", $4j)
If @error Then Return SetError(@error, @extended, False)
Return $3s[0]
EndFunc
...
EndIf
Local $3s = dllcall($1, "bool", "SetLayeredWindowAttributes", "hwnd", $3t, "dword", $4y, "byte", $4z, "dword", $50)
If @error Then Return SetError(@error, @extended, False)
Return $3s[0]
EndFunc
Func _m($52, $53, $54, $55 = 0)
Local $3s = dllcall($1, "handle", "SetWindowsHookEx", "int", $52, "ptr", $53, "handle", $54, "dword", $55)
If @error Then Return SetError(@error, @extended, 0)
Return $3s[0]
EndFunc
Func _n($4b)
Local $3s = dllcall($1, "bool", "UnhookWindowsHookEx", "handle", $4b)
If @error Then Return SetError(@error, @extended, False)
Return $3s[0]
EndFunc
Func _o($56, $57, $58, $59, $5a = 0, $5b = 0)
Local $5c = dllcall($2, "bool", "AdjustTokenPrivileges", "handle", $56, "bool", $57, "struct*", $58, "dword", $59, "struct*", $5a, "struct*", $5b)
If @error Then Return SetError(1, @extended, False)
Return Not($5c[0] = 0)
EndFunc
Func _p($5d = $3m)
Local $5c = dllcall($2, "bool", "ImpersonateSelf", "int", $5d)
If @error Then Return SetError(1, @extended, False)
Return Not($5c[0] = 0)
EndFunc
Func _q($5e, $5f)
Local $5c = dllcall($2, "bool", "LookupPrivilegeValueW", "wstr", $5e, "wstr", $5f, "int64*", 0)
...
Local $5c = dllcall($2, "bool", "OpenThreadToken", "handle", $5h, "dword", $5g, "bool", $5i, "handle*", 0)
...
dllcall($0, "bool", "CloseHandle", "handle", $5v)
If @error Then Return SetError(@error, @extended, False)
Return $5w
EndFunc
Func _v($5x, $5y = 0)
Local $3s = dllcall($0, "handle", "GlobalAlloc", "uint", $5y, "ulong_ptr", $5x)
If @error Then Return SetError(@error, @extended, 0)
Return $3s[0]
EndFunc
Func _w($5z)
Local $3s = dllcall($0, "ptr", "GlobalLock", "handle", $5z)
If @error Then Return SetError(@error, @extended, 0)
Return $3s[0]
EndFunc
Func _x($5z)
Local $3s = dllcall($0, "bool", "GlobalUnlock", "handle", $5z)
If @error Then Return SetError(@error, @extended, 0)
Return $3s[0]
EndFunc
...
Local $3s = dllcall($0, "bool", "ReadProcessMemory", "handle", DllStructGetData($5t, "hProc"), _
...
Local $3s = dllcall($0, "bool", "WriteProcessMemory", "handle", DllStructGetData($5t, "hProc"), _
"ptr", $63, $64, $62, "ulong_ptr", $60, "ulong_ptr*", 0)
If @error Then Return SetError(@error, @extended, False)
Return $3s[0]
EndFunc
Func _11($5v, $65, $60, $66, $67)
Local $3s = dllcall($0, "ptr", "VirtualAllocEx", "handle", $5v, "ptr", $65, "ulong_ptr", $60, "dword", $66, "dword", $67)
If @error Then Return SetError(@error, @extended, 0)
Return $3s[0]
EndFunc
Func _12($5v, $65, $60, $68)
Local $3s = dllcall($0, "bool", "VirtualFreeEx", "handle", $5v, "ptr", $65, "ulong_ptr", $60, "dword", $68)
If @error Then Return SetError(@error, @extended, False)
Return $3s[0]
EndFunc
Func _13($5g, $69, $4p, $6a = False)
Local $3s = dllcall($0, "handle", "OpenProcess", "dword", $5g, "bool", $69, "dword", $4p)
If @error Then Return SetError(@error, @extended, 0)
...
Local $6d = 0
If Not @error Then
$3s = dllcall($0, "handle", "OpenProcess", "dword", $5g, "bool", $69, "dword", $4p)
$6b = @error
$6c = @extended
...
EndIf
EndIf
dllcall($0, "bool", "CloseHandle", "handle", $56)
...
Func _1q($8f = 8)
Local $3s = dllcall($1, "handle", "CreateMenu")
If @error Then Return SetError(@error, @extended, 0)
...
EndFunc
Func _1r($8f = 8)
Local $3s = dllcall($1, "handle", "CreatePopupMenu")
If @error Then Return SetError(@error, @extended, 0)
If $3s[0] = 0 Then Return SetError(10, 0, 0)
_1z($3s[0], $8f)
Return $3s[0]
EndFunc
Func _1s($8g)
Local $3s = dllcall($1, "bool", "DestroyMenu", "handle", $8g)
If @error Then Return SetError(@error, @extended, False)
Return $3s[0]
EndFunc
...
Local $3s = dllcall($1, "bool", "GetMenuItemInfo", "handle", $8g, "uint", $6u, "bool", $8h, "struct*", $8i)
If @error Then Return SetError(@error, @extended, 0)
...
EndIf
Local $3s = dllcall($1, "bool", "InsertMenuItemW", "handle", $8g, "uint", $4k, "bool", True, "struct*", $8l)
If @error Then Return SetError(@error, @extended, False)
Return $3s[0]
EndFunc
Func _1w($8g, $6u, ByRef $8i, $8h = True)
DllStructSetData($8i, "Size", DllStructGetSize($8i))
Local $3s = dllcall($1, "bool", "SetMenuItemInfoW", "handle", $8g, "uint", $6u, "bool", $8h, "struct*", $8i)
If @error Then Return SetError(@error, @extended, False)
Return $3s[0]
EndFunc
...
DllStructSetData($8i, "Size", DllStructGetSize($8i))
Local $3s = dllcall($1, "bool", "SetMenuInfo", "handle", $8g, "struct*", $8i)
If @error Then Return SetError(@error, @extended, False)
Return $3s[0]
EndFunc
Func _1z($8g, $8f)
...
$5y = BitOR($5y, $85)
EndSwitch
Local $3s = dllcall($1, "bool", "TrackPopupMenu", "handle", $8g, "uint", $5y, "int", $74, "int", $75, "int", 0, "hwnd", $3t, "ptr", 0)
If @error Then Return SetError(@error, @extended, 0)
Return $3s[0]
EndFunc
Global Const $8s = _22()
...
Local $8y = dllcall($0, 'int', 'GetVersionExW', 'ptr', DllStructGetPtr($8x))
...
If @error Then Return $ce
Local $cj = dllcall($1, "handle", "GetDC", "hwnd", $3t)
If @error Then Return $ce
$cj = $cj[0]
Local $ck = dllcall($3, "int", "GetDeviceCaps", "handle", $cj, "int", 90)
If Not @error Then
$ck = $ck[0]
$ce[0] = Int(2 *(.25 - DllStructGetData($ch, 1) * 72 / $ck)) / 2
EndIf
dllcall($1, "int", "ReleaseDC", "hwnd", $3t, "handle", $cj)
...
_x($d4)
$3s = dllcall($4, "int", "CreateStreamOnHGlobal", "handle", $d5, "int", True, "ptr*", 0)
...
dllcall($5, "long", "DispCallFunc", "ptr", $d7, "dword", 8 + 8 * @AutoItX64, _
...
$dc = dllcall($3, 'ptr', 'CreateDIBSection', 'hwnd', 0, 'ptr', DllStructGetPtr($da), 'uint', 0, 'ptr*', 0, 'ptr', 0, 'dword', 0)
If(Not @error) And($dc[0]) Then
dllcall($3, 'dword', 'SetBitmapBits', 'ptr', $dc[0], 'dword', $8y[2] * $8y[3] * 4, 'ptr', DllStructGetData($9a, 'Scan0'))
$dc = $dc[0]
Else
$dc = 0
EndIf
...
Local $dg = dllcall($6, "int", "CryptStringToBinary", "str", $de, "int", 0, "int", 1, "ptr", 0, "ptr", DllStructGetPtr($df, 1), "ptr", 0, "ptr", 0)
If @error Or Not $dg[0] Then Return SetError(1, 0, "")
Local $dh = DllStructCreate("byte[" & DllStructGetData($df, 1) & "]")
$dg = dllcall($6, "int", "CryptStringToBinary", "str", $de, "int", 0, "int", 1, "ptr", DllStructGetPtr($dh), "ptr", DllStructGetPtr($df, 1), "ptr", 0, "ptr", 0)

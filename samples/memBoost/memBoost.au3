...
; Aut2Exe Settings
;===============================================================================================================
#AutoIt3Wrapper_Icon=Themes\Icons\memBoost.ico					;~ Filename of the Ico file to use for the compiled exe
#AutoIt3Wrapper_OutFile_Type=exe								;~ exe=Standalone executable (Default); a3x=Tokenised AutoIt3 code file
#AutoIt3Wrapper_OutFile=memBoost.exe							;~ Target exe/a3x filename.
#AutoIt3Wrapper_OutFile_X64=memBoost_X64.exe					;~ Target exe filename for X64 compile.
...
#AutoIt3Wrapper_Res_Description=Bemory Booster     				;~ Description field
#AutoIt3Wrapper_Res_Fileversion=3.0.0.2306
...
; Remove resources
#AutoIt3Wrapper_Res_Icon_Add=Themes\Icons\memBoostH.ico					; 201
...
Global $g_hGraphMem
Global $g_aGraphMemLine[4]
Global $g_hLblMemUsage, $g_hLblPageUsage, $g_hLblMemFree
Global $g_hProgMemUsage1, $g_hProgMemUsage2
Global $g_hProgPageUsage1, $g_hProgPageUsage2
Global $g_hProgMemFree1, $g_hProgMemFree2
Global $g_iMinDiff = 0
Global $g_iMemPeakPerc = 0

Global $g_aBuffers[100]
Global $g_aHeatMap[24][60][3]
...
Func _StartCoreGui()
	...
	$g_hCoreGui = GUICreate($g_sProgramTitle, $g_iCoreGuiWidth, $g_iCoreGuiHeight, -1, 25, -1)
	If Not @Compiled Then GUISetIcon($g_aCoreIcons[0])
	GUISetBkColor($g_hCoreGui, 0x151C23)
	...
	GUICtrlCreateLabel("", -10, 98, 620, 1)
	GUICtrlSetBkColor(-1, 0xFFFFFF)
	GUICtrlCreateLabel("", -10, 99, 620, 1)
	GUICtrlSetBkColor(-1, 0x00ACFF)
	GUICtrlCreateLabel("", -10, 100, 635, 600)
	GUICtrlSetBkColor(-1, 0x151C23)

	GUICtrlCreateLabel("", 10, 110, 1, 280)
	GUICtrlSetBkColor(-1, 0x555555)

	GUICtrlCreateLabel("", 10, 110, 595, 1)
	GUICtrlSetBkColor(-1, 0x555555)

	GUICtrlCreateLabel("PERFORMANCE INFORMATION", 16, 114, 400, 15)
	GUICtrlSetFont(-1, 8)
	GUICtrlSetColor(-1, 0x999999)
	GUICtrlSetBkColor(-1, 0x141B22)

	GUICtrlCreateLabel("", 10, 130, 595, 1)
	GUICtrlSetBkColor(-1, 0x555555)

	$g_hGraphMem = _SLG_CreateGraph($g_hCoreGui, 13, 132, 201, 118, 1, 100, 100, 0xFF00ACFF, 1, -1, True)

	$g_aGraphMemLine[0] = _SLG_AddLine($g_hGraphMem, 0xFFFFFF00, 1)
	$g_aGraphMemLine[1] = _SLG_AddLine($g_hGraphMem, 0xFF5DB60C, 1)
	$g_aGraphMemLine[2] = _SLG_AddLine($g_hGraphMem, 0xFFFF8000, 1)
	$g_aGraphMemLine[3] = _SLG_AddLine($g_hGraphMem, 0xFFB035D4, 1)
	_SLG_UpdateGraph($g_hGraphMem)

	GUICtrlCreateLabel("", 216, 130, 1, 260)
	GUICtrlSetBkColor(-1, 0x555555)

	$g_hLblMemUsage = GUICtrlCreateLabel("", 230, 140, 295, 15)
	GUICtrlSetFont($g_hLblMemUsage, 8)
	GUICtrlSetColor($g_hLblMemUsage, 0x00ACFF)
	GUICtrlSetBkColor($g_hLblMemUsage, 0x141B22)
	GUICtrlCreateLabel("", 230, 155, 307, 15)
	GUICtrlSetBkColor(-1, 0x00ACFF)
	GUICtrlCreateLabel("", 231, 156, 305, 13)
	GUICtrlSetBkColor(-1, 0x002D52)
	$g_hProgMemUsage1 = GUICtrlCreateLabel("", 232, 157, 303, 11)
	$g_hProgMemUsage2 = GUICtrlCreateLabel("", 233, 158, 301, 9)
	GUICtrlSetBkColor($g_hProgMemUsage1, 0x008BCC)
	GUICtrlSetBkColor($g_hProgMemUsage2, 0x00ACFF)
	GUICtrlCreateLabel("32000 MB", 525, 140, 70, 15, $SS_RIGHT)
	GUICtrlSetFont(-1, 8)
	GUICtrlSetColor(-1, 0x00ACFF)
	GUICtrlSetBkColor(-1, 0x141B22)
	GUICtrlCreateLabel("100%", 545, 155, 50, 15, $SS_RIGHT)
	GUICtrlSetFont(-1, 8)
	GUICtrlSetColor(-1, 0x00ACFF)
	GUICtrlSetBkColor(-1, 0x141B22)

	$g_hLblPageUsage = GUICtrlCreateLabel("PAGING:", 230, 175, 280, 15)
	GUICtrlSetFont($g_hLblPageUsage, 8)
	GUICtrlSetColor($g_hLblPageUsage, 0xFFFF00)
	GUICtrlSetBkColor($g_hLblPageUsage, 0x141B22)
	GUICtrlCreateLabel("", 230, 190, 307, 15)
	GUICtrlSetBkColor(-1, 0xFFFF00)
	GUICtrlCreateLabel("", 231, 191, 305, 13)
	GUICtrlSetBkColor(-1, 0x363600)
	$g_hProgPageUsage1 = GUICtrlCreateLabel("", 232, 192, 50, 11)
	$g_hProgPageUsage2 = GUICtrlCreateLabel("", 233, 193, 48, 9)
	GUICtrlSetBkColor($g_hProgPageUsage1, 0xCBCB00)
	GUICtrlSetBkColor($g_hProgPageUsage2, 0xFFFF00)
	GUICtrlCreateLabel("32000 MB", 525, 175, 70, 15, $SS_RIGHT)
	GUICtrlSetFont(-1, 8)
	GUICtrlSetColor(-1, 0xFFFF00)
	GUICtrlSetBkColor(-1, 0x141B22)
	GUICtrlCreateLabel("100%", 545, 190, 50, 15, $SS_RIGHT)
	GUICtrlSetFont(-1, 8)
	GUICtrlSetColor(-1, 0xFFFF00)
	GUICtrlSetBkColor(-1, 0x141B22)

	$g_hLblMemFree = GUICtrlCreateLabel("FREE:", 230, 210, 280, 15)
	GUICtrlSetFont($g_hLblMemFree, 8)
	GUICtrlSetColor($g_hLblMemFree, 0x5DB60C)
	GUICtrlSetBkColor($g_hLblMemFree, 0x141B22)
	GUICtrlCreateLabel("", 230, 225, 280, 15)
	GUICtrlSetBkColor(-1, 0x5DB60C)
	GUICtrlCreateLabel("", 231, 226, 278, 13)
	GUICtrlSetBkColor(-1, 0x141B22)
	$g_hProgMemFree1 = GUICtrlCreateLabel("", 232, 227, 50, 11)
	$g_hProgMemFree2 = GUICtrlCreateLabel("", 233, 228, 48, 9)
	GUICtrlSetBkColor($g_hProgMemFree1, 0x4F990B)
	GUICtrlSetBkColor($g_hProgMemFree2, 0x5DB60C)
	...
	GUICtrlCreateLabel("PAGING:", 230, 245, 280, 15)
	GUICtrlSetFont(-1, 8)
	GUICtrlSetColor(-1, 0xB858D8)
	GUICtrlSetBkColor(-1, 0x141B22)
	GUICtrlCreateLabel("", 230, 260, 280, 15)
	GUICtrlSetBkColor(-1, 0xB858D8)
	GUICtrlCreateLabel("", 231, 261, 278, 13)
	GUICtrlSetBkColor(-1, 0x141B22)
	GUICtrlCreateLabel("", 232, 262, 50, 11)
	GUICtrlSetBkColor(-1, 0xB035D4)
	GUICtrlCreateLabel("", 233, 263, 48, 9)
	GUICtrlSetBkColor(-1, 0xB858D8)


	GUICtrlCreateLabel("COMMITTED:", 230, 280, 280, 15)
	GUICtrlSetFont(-1, 8)
	GUICtrlSetColor(-1, 0xFF8000)
	GUICtrlSetBkColor(-1, 0x141B22)
	GUICtrlCreateLabel("", 230, 295, 280, 15)
	GUICtrlSetBkColor(-1, 0xFF8000)
	GUICtrlCreateLabel("", 231, 296, 278, 13)
	GUICtrlSetBkColor(-1, 0x141B22)
	GUICtrlCreateLabel("", 232, 297, 50, 11)
	GUICtrlSetBkColor(-1, 0xDD6F00)
	GUICtrlCreateLabel("", 233, 298, 48, 9)
	GUICtrlSetBkColor(-1, 0xFF8000)

	GUICtrlCreateLabel("", 10, 325, 595, 1)
	GUICtrlSetBkColor(-1, 0x555555)
	GUICtrlCreateLabel("", 10, 345, 595, 1)
	GUICtrlSetBkColor(-1, 0x555555)

	GUICtrlCreateLabel("", 230, 360, 307, 127)
	GUICtrlSetBkColor(-1, 0x00ACFF)
	GUICtrlCreateLabel("", 231, 361, 305, 125)
	GUICtrlSetBkColor(-1, 0x141B22)

	For $hh = 0 to 23
		For $hm = 0 to 59
			$g_aHeatMap[$hh][$hm][0] = GUICtrlCreateLabel("", 234 + ($hm * 5), 364 + ($hh * 5), 4, 4)
			GUICtrlSetBkColor($g_aHeatMap[$hh][$hm][0], 0x333333)
		Next
	Next
	...
	GUICtrlCreateLabel("", 605, 110, 1, 280)
	GUICtrlSetBkColor(-1, 0x555555)
...
Func _GraphLineColors()


EndFunc


Func _GetHeatMapColor($iPerc)

	Local $iCleanPerc

	Local $aHeatMapColors[100] = [	0x008000, 0x058200, 0x0A8500, 0x108800, 0x148A00, 0x1A8D00, 0x1F8F00, 0x259100, 0x2A9500, 0x2F9700, _
									0x349900, 0x399C00, 0x3E9E00, 0x43A100, 0x48A300, 0x4EA600, 0x53A900, 0x58AB00, 0x5CAE00, 0x63B100, _
									0x68B300, 0x6CB600, 0x71B800, 0x77BA00, 0x7CBE00, 0x81C000, 0x86C200, 0x8CC500, 0x91C700, 0x95CA00, _
									0x9ACC00, 0xA0CF00, 0xA5D200, 0xAAD400, 0xAFD700, 0xB5DA00, 0xBADC00, 0xBEDF00, 0xC3E100, 0xC9E400, _
									0xCEE600, 0xD3E800, 0xD8EC00, 0xDDEE00, 0xE2F000, 0xE8F300, 0xECF500, 0xECF500, 0xF7FB00, 0xFCFE00, _
									0xFFFD00, 0xFFF700, 0xFFF200, 0xFFEC00, 0xFFE700, 0xFFE200, 0xFFDE00, 0xFFD900, 0xFFD300, 0xFFCE00, _
									0xFFC900, 0xFFC400, 0xFFBF00, 0xFFBA00, 0xFFB500, 0xFFAF00, 0xFFAA00, 0xFFA500, 0xFFA100, 0xFF9C00, _
									0xFF9600, 0xFF9200, 0xFF8D00, 0xFF8700, 0xFF8200, 0xFF7D00, 0xFF7800, 0xFF7200, 0xFF6D00, 0xFF6900, _
									0xFF6400, 0xFF5E00, 0xFF5900, 0xFF5400, 0xFF4F00, 0xFF4F00, 0xFF4400, 0xFF4000, 0xFF3B00, 0xFF3500, _
									0xFF3000, 0xFF2B00, 0xFF2600, 0xFF2100, 0xFF1C00, 0xFF1600, 0xFF1200, 0xFF0C00, 0xFF0700, 0xFF0200]

	$iCleanPerc = Int($iPerc) - 1
	Return $aHeatMapColors[$iCleanPerc]
...
Func _UpdateMemoryStats()
	...
	$g_aMemStats = _WinAPI_GetPerformanceInfo()

	Local $g_aPageStats = MemGetStats()
	Local $iPageTotal = Floor($g_aPageStats[3] / 1024)
	Local $iPageAvailable = Floor($g_aPageStats[4] / 1024)
	Local $iPageUsage = Round($iPageTotal - $iPageAvailable)
	Local $iPageUsagePerc = Round(($iPageUsage / $iPageTotal) * 100)

	Local $iMemTotal = Floor($g_aMemStats[3] / 1024 / 1024) ; Physical Memory (MB)
	Local $iMemAvailable = Floor($g_aMemStats[4] / 1024 / 1024)
	Local $iCached = Floor($g_aMemStats[5] / 1024 / 1024)
	Local $iCachedPerc = Round(($iCached / $iMemTotal) * 100)
	Local $iMemUsed = $iMemTotal - $iMemAvailable
	Local $iMemUsedPerc = Round(($iMemUsed / $iMemTotal) * 100)
	Local $iMemFree = Floor($g_aMemStats[6] / 1024 / 1024)
	Local $iMemFreePerc = Round(($iMemFree / $iMemTotal) * 100)

	If $iMemAvailable <> $g_aBuffers[1] Then
		GUICtrlSetData($g_hLblMemUsage, StringUpper(StringFormat("RAM: %d MB USED OF %d MB (%d%)", _
			$iMemUsed, $iMemTotal, $iMemUsedPerc)))
		$g_aBuffers[1] = $iMemAvailable
	EndIf

	If $iMemUsedPerc <> $g_aBuffers[2] Then
...
;~ 		EndIf
		...
		_ProgressBar_SetData($g_hCoreGui, $g_hProgMemUsage1, $g_hProgMemUsage2, 232, 157, 281, $iMemUsedPerc)
		$g_aBuffers[2] = $iMemUsedPerc

	EndIf

	If $iPageUsage <> $g_aBuffers[3] Then
		GUICtrlSetData($g_hLblPageUsage, StringUpper(StringFormat("CACHED: %d MB OF %d MB (%d%)", _
			$iCached, $iMemTotal, $iCachedPerc)))
		$g_aBuffers[3] = $iPageUsage
	EndIf

	If $iMemFree <> $g_aBuffers[5] Then
		GUICtrlSetData($g_hLblMemFree, StringUpper(StringFormat("FREE: %d MB OF %d MB (%d%)", _
			$iMemFree, $iMemTotal, $iMemFreePerc)))
		$g_aBuffers[5] = $iMemFree
	EndIf
...
;~ 		_SLG_SetLineValue($g_hGraphMem, 1)
;~ 		_SLG_SetLineValue($g_hGraphMem, 1, $g_aGraphMemLine[0])
;~ 		_SLG_SetLineValue($g_hGraphMem, 1, $g_aGraphMemLine[1])
;~ 		_SLG_SetLineValue($g_hGraphMem, 1, $g_aGraphMemLine[2])
;~ 		_SLG_SetLineValue($g_hGraphMem, 1, $g_aGraphMemLine[3])
;~ 		_SLG_UpdateGraph($g_hGraphMem)
;~ 		$g_iFirstRun = 0
;~ 		Return
	EndIf

	_SLG_SetLineValue($g_hGraphMem, $iMemUsedPerc)
	_SLG_SetLineValue($g_hGraphMem, $iCachedPerc, $g_aGraphMemLine[0])
	_SLG_SetLineValue($g_hGraphMem, $iMemFreePerc, $g_aGraphMemLine[1])
	_SLG_SetLineValue($g_hGraphMem, 10, $g_aGraphMemLine[2])
	_SLG_SetLineValue($g_hGraphMem, 20, $g_aGraphMemLine[3])
	_SLG_UpdateGraph($g_hGraphMem)


	If $g_iMemPeakPerc < $iMemUsedPerc Then
		$g_iMemPeakPerc = $iMemUsedPerc
	EndIf

	If $g_iMinDiff <> @MIN Then
		GUICtrlSetBkColor($g_aHeatMap[Int(@HOUR)][Int(@MIN)][0], _GetHeatMapColor($g_iMemPeakPerc))
		$g_iMinDiff = @MIN
		$g_iMemPeakPerc = 0
	EndIf
...
Func _ReduceMemory()
	...
	Local $g_aMemStats = MemGetStats()

	If $g_aMemStats[0] > $g_iMaxSysMemoryPerc And $g_iReduceMemory = 1 Then

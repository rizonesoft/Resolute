...
#AutoIt3Wrapper_Res_Fileversion=1.6.8.1771
...
Global Const $MBTitle = "Memory Booster Professional", $MBver = FileGetVersion(@ScriptFullPath)
Global Const $MBSettings = @ScriptDir & "\memBoostPro.ini"
Global Const $MBHelpFile = @ScriptDir & "\Support\memBoost.chm"

Global Const $binCC32exe = @ScriptDir & "\Bin\cc32.exe"
Global Const $binCC64exe = @ScriptDir & "\Bin\cc64.exe"
Global Const $binClnMexe = @ScriptDir & "\Bin\cleanmem.exe"

;~ Settings
;~ Global $OSVer
Global $Initializing = 1, $SetStartMin, $SetOnTop, $SetNotify, $SetOpMeth, $SetAutoCount
Global $SetPlSounds, $SetPlWarn, $SetWarnEvery, $SetWarnLoad, $SetCacheClear, $OSG

Global $MemInfo, $MemDiff, $Count = 0, $TotalCount
Global $memBoostGUI, $prMemUs, $grMemUs, $btnOptimize, $GraMS, $G1Line2, $G1Line3, $grCPU, $GraCPU, $prCPU
...
Global $radIntel, $radAuto, $radNoAuto, $comboAutSec, $chkClearCache, $BtnOptApply
Global $chkPlSounds, $chkPlWarn, $cmPlWarnEvery, $cmPlWarnLoad
Global $warnCount = 0
...
If $OSG = "WIN8" Then $SetCacheClear = IniRead($MBSettings, "Advanced", "ClearSystemCacheOptimize", 0)
...
Func _StartMainGUI()
	...
	Local $mPro, $miClearCache
	Local $btnClearCache, $btnClose, $btnHelp
	...
	$miHome = GUICtrlCreateMenuItem("Datum Home", $mHelp)
	GUICtrlCreateMenuItem("", $mHelp)
	...
	GUICtrlSetOnEvent($miHome, "_OpenDatumHomePage")
	...
	$mPro = GUICtrlCreateMenu("&Pro")
	$miClearCache = GUICtrlCreateMenuItem("&Clean memory", $mPro)

	GUICtrlSetOnEvent($miClearCache, "_ClearSysCacheMemItemPressed")
	...
	$btnClearCache = GUICtrlCreateButton("Clean", 180, 70, 115, 35)
	$btnOptions = GUICtrlCreateButton("Options...", 300, 70, 115, 35)

	GUICtrlSetOnEvent($btnOptimize, "_OptimizeItemPressed")
	GUICtrlSetOnEvent($btnClearCache, "_ClearSysCacheMemItemPressed")
...
Func _TimerStuff()
	...
	Else
		...
		Else
			...
			$TotalCount = $Count
			If Not $Initializing Then _ClearProcessesWorkingSet(1)
...
Func _OptimizeItemPressed()
	_ClearProcessesWorkingSet(0)
EndFunc


Func _ClearProcessesWorkingSet($iFlag = 1)
	...
	If Not (@error Or @extended) Then
		...
		If $SetCacheClear And $OSG = "WIN8" Then
			_ClearSystemCache(1)
		EndIf
		If Not $iFlag Then
			If $SetPlSounds Then
...
Func _ClearSysCacheMemItemPressed()
	_ClearSystemCache(0)
EndFunc


Func _ClearSystemCache($iFlag = 0)

	If $OSG = "WIN6" Then
		If Not $iFlag Then ShellExecute($binClnMexe)
	Else
		_ClearSystemCacheNew($iFlag)
	EndIf

EndFunc


Func _ClearSystemCacheNew($iFlag = 0)

	If Not ProcessExists("cc32.exe") Then
		If FileExists($binCC32exe) Then
			ShellExecute($binCC32exe, "flush")
			If Not @error Then
				_ClearSystemCacheMessage($iFlag)
			EndIf
		EndIf
	EndIf
...
Func _ClearSystemCacheMessage($iFlag = 0)

	If Not $iFlag Then
		If $SetPlSounds Then
			SoundPlay("")
			SoundPlay(@ScriptDir & "\Sounds\System-Cache-Cleared.mp3", 0)
		EndIf
		If $SetNotify Then MsgBox(262208, "Cache gone!", "System cache cleared out successfully.", 60)
	EndIf
...
Func _CloseMemoryBooster()
	...
	Local $cc32PID, $cc64PID, $ccNTPID

	TraySetState(2)

	$cc32PID = ProcessExists("cc-32.exe") ; Will return the PID or 0 if the process isn't found.
	If $cc32PID Then ProcessClose($cc32PID)
	$cc64PID = ProcessExists("cc-64.exe") ; Will return the PID or 0 if the process isn't found.
	If $cc64PID Then ProcessClose($cc64PID)
	$ccNTPID = ProcessExists("cc-NT.exe") ; Will return the PID or 0 if the process isn't found.
	If $ccNTPID Then ProcessClose($ccNTPID)
...
Func _OptionsDlg()
	...
	$OptionsDlg = GUICreate($MBTitle & " Options", 500, 500, -1, -1, BitOr($WS_CAPTION, $WS_POPUPWINDOW), $WS_EX_TOPMOST)
	...
	GUICtrlCreateGroup("Sounds", 15, 230, 470, 90)
	...
	GUICtrlCreateGroup("Advanced", 15, 330, 470, 90)
	$chkClearCache = GUICtrlCreateCheckbox(" Also clear system cache when optimizing", 25, 355, 350, 15)
	GUICtrlCreateGroup("", -99, -99, 1, 1)  ;~ Close group
	...
	If $OSG = "WIN8" Then
		If $SetCacheClear = 1 Then GUICtrlSetState($chkClearCache, $GUI_CHECKED)
	EndIf
	...
	GUICtrlSetOnEvent($chkClearCache, "_EnableApplyButton")
	...
	If $OSG = "WIN6" Then
		GUICtrlSetState($chkClearCache, $GUI_UNCHECKED)
		GuiCtrlSetState($chkClearCache, $GUI_DISABLE)
	EndIf
...
Func _OptionsApplyButtonPressed()
	...
	If $OSG = "WIN8" Then
		If GUICtrlRead($chkClearCache) = $GUI_CHECKED Then
			$SetCacheClear = 1
		ElseIf GUICtrlRead($chkClearCache) = $GUI_UNCHECKED Then
			$SetCacheClear = 0
		EndIf
	EndIf
	...
	If $OSG = "WIN8" Then IniWrite($MBSettings, "Advanced", "ClearSystemCacheOptimize", $SetCacheClear)
...
Func _AboutDlg()
	...
	GUICtrlSetOnEvent($abHome, "_OpenDatumHomePage")
...
Func _DonateSomething()
	ShellExecute("https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=V22NKZ6YLFK8L")
EndFunc


Func _OpenDatumHomePage()
	ShellExecute("https://www.datum-forensics.com")
EndFunc


Func _Support()
	ShellExecute("https://www.datum-forensics.com/contact/")
EndFunc


Func _NoNonsense()
	ShellExecute("https://www.datum-forensics.com/about/legal-stuff/no-nonsense/")
EndFunc


Func _SubscribeInReader()
	ShellExecute("https://www.datum-forensics.com/feed/")
EndFunc


Func _OpenFacebook()
	ShellExecute("http://www.facebook.com/datumsa")
EndFunc


Func _FollowOnTwitter()
	ShellExecute("http://twitter.com/datum9a")
EndFunc
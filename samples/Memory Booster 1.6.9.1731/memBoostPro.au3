...
#AutoIt3Wrapper_Res_Fileversion=1.6.8.1753
...
Global Const $binCC32exe = @ScriptDir & "\Bin32\cc32.exe"
Global Const $binCC64exe = @ScriptDir & "\Bin64\cc64.exe"
Global Const $binClnMexe = @ScriptDir & "\Bin32\cleanmem.exe"
...
Func _StartMainGUI()
	...
	$miClearCache = GUICtrlCreateMenuItem("&Clear out System Cache", $mTools)
	...
	Select
		Case $iMsgBoxAnswer = 1 ;OK
			...
			If @OSVersion = "WIN_2003" Or @OSVersion = "WIN_XP" Or @OSVersion = "WIN_XPe" Then
				...
				If Not FileExists(@ScriptDir & "\~tmp") Then DirCreate(@ScriptDir & "\~tmp")
				Local $tlM = Round($memStruct[1] / 2.1) * 1024
				Local $ovbs = FileOpen(@ScriptDir & "\~tmp\memBoost.vbs", 2)
				FileWrite($ovbs, "Space(" & $tlM & ")")
				FileClose($ovbs)
				ShellExecute("WScript.exe", Chr(34) & @ScriptDir & "\~tmp\memBoost.vbs" & Chr(34))

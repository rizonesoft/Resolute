...
#AutoIt3Wrapper_Res_Fileversion=23.0.0.109
...
#AutoIt3Wrapper_Res_Icon_Add=..\..\Resources\Icons\Menus\ComIntRep.ico			; 277
#AutoIt3Wrapper_Res_Icon_Add=..\..\Resources\Icons\Menus\DVDRepair.ico			; 278
#AutoIt3Wrapper_Res_Icon_Add=..\..\Resources\Icons\Menus\USBRepair.ico			; 279
...
Global $g_sUrlSupport			= "https://www.rizonesoft.com/contact-us/|www.rizonesoft.com/contact-us/"						; https://www.rizonesoft.com/contact
...
Func _StartCoreGui()
	...
	$g_hMenuNetwork = GUICtrlCreateMenu($g_aLangMenus[14])
	$g_hMenuRepair = GUICtrlCreateMenu($g_aLangMenus[17])
	...
	GUICtrlCreateGroup("System Repair", 420, 10, 200, 100)
	GUICtrlSetResizing(-1, BitOR($GUI_DOCKRIGHT, $GUI_DOCKTOP, $GUI_DOCKSIZE))
	$hBtnScanSysFiles = GUICtrlCreateButton("Scan System Files", 430, 30, 180, 33)
	GUICtrlSetResizing(-1, BitOR($GUI_DOCKRIGHT, $GUI_DOCKTOP, $GUI_DOCKSIZE))
	$hBtnDISM = GUICtrlCreateButton("Restore Health", 430, 63, 180, 33)
	GUICtrlSetResizing(-1, BitOR($GUI_DOCKRIGHT, $GUI_DOCKTOP, $GUI_DOCKSIZE))
...
Func _StartComIntRep()
	_ExecuteResoluteTool("ComIntRep")
EndFunc

Func _StartDVDRepair()
	_ExecuteResoluteTool("DVDRepair")
EndFunc

Func _StartUSBRepair()
	_ExecuteResoluteTool("USBRepair")
EndFunc

#EndRegion "Run Programs"
...
Func _ExecuteResoluteTool($sExeName)

	Local $s64ExePath = $g_sResolutePath & $sExeName & "_X64.exe"
	Local $s86ExePath = $g_sResolutePath & $sExeName & ".exe"

	If @OSArch = "X64" Then
		If FileExists($s64ExePath) Then
			ShellExecute($s64ExePath)
			Return
		Else
			If FileExists($s86ExePath) Then
				ShellExecute($s86ExePath)
				Return
			EndIf
		EndIf
	Else
...
Func _ShellExecuteEx($sExePath, $sName, $sParameters, $sWorkingDir = "")
	...
	ShellExecute($sExePath, $sParameters, $sWorkingDir)
	If @error Then
		_Logging_EditWrite(_Logging_SetLevel(StringFormat($g_aLangMessages2[1], $sName), "ERROR"))
	EndIf
	_Logging_FinalMessage(StringFormat($g_aLangMessages2[2], $sName))
...
Func _SetResources()
	...
	Else
		...
		$g_aMenuIcons[8] = $g_sThemesDir & "\Icons\Menus\ComIntRep.ico"

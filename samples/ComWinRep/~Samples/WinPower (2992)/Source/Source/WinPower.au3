...
Global Const $PowerSettings = @ScriptDir & "\WinPower.ini"
...
$PowerLogSize = IniRead($PowerSettings, "Logging", "Size", 2)
$HideOnMinimize = IniRead($PowerSettings, "GUI", "HideOnMinimize", 1)

Global Const $GBinDir = @ScriptDir & "\Bin"
Global Const $GLogDir = @ScriptDir & "\Logging"
Global Const $GThemeDir = @ScriptDir & "\Themes"
Global Const $GBacDir = @ScriptDir & "\Backup"
Global Const $GPowerLog = $GLogDir & "\WinPower.log"
Global Const $GNote2Exe = $GBinDir & "\Notepad2\Notepad2.exe"
...
If Not FileExists($GThemeDir) Then DirCreate($GThemeDir)
If Not FileExists($GBinDir) Then DirCreate($GThemeDir)

_CreateLoggingDirectory()
_LoaderUpdate(2)
If @Compiled Then
	;FileInstall("WinPower.dll", $PowerResDll, 0)
	FileInstall("Command.bat", @ScriptDir & "\Command.bat", 0)
	FileInstall("Themes\Process.ani", $GThemeDir & "\Process.ani", 0)
EndIf
...
If FileExists($GPowerLog) Then
	...
	Else
		If FileGetSize($GPowerLog) > ($PowerLogSize * 1024) * 1024 Then
...
If @OSVersion = "WIN_2000" Then
	MsgBox(64, $PowerTitle, "This utility is not compatable with your version of windows. " & _
							"If you believe this to be an error, please feel free to email me at " & _
							"rizonetech@gmail.com with all the details.", 30)
	Exit
EndIf
...
$FiLogMenu = GUICtrlCreateMenu("&Logging", $FileMenu)

...
; This is the modified _Logging_EditWrite function
Func _Logging_EditWrite($sMessage = "", $bTimePrex = True, $bToLogFile = True)
    ...
    If $g_ShowInterface Then
        For $i = 1 To $aLines[0] ; Loop through each line
            ...
            Local $iImage = _DetermineLogImage($sLine) ; Determine the image index based on the line content
            _GUICtrlListView_AddItem($g_hListStatus, Chr(32) & $sTimeStamp & $sLine, $iImage) ; Add each line as a separate item
...
Func _DetermineLogImage($sLine)
    Local $iImage = 0
		...
		If StringLeft($sLine, $iErrorLen) = $g_aLangLogging[32] Or _
				StringLeft($sLine, 6) = "Error:" Or _
				__Logging_ValidateError($sLine) Then
			$iImage = 2
			$g_iLoggingErrors += 1
		ElseIf StringLeft($sLine, $iSuccessLen) = $g_aLangLogging[34] Or _
				StringLeft($sLine, 8) = "Success:" Or _
				__Logging_ValidateSuccess($sLine) Then
			$iImage = 1
		ElseIf StringLeft($sLine, $iFinalLen) = $g_aLangLogging[0] Or _
				StringLeft($sLine, 8) = "Finished" Then
			Switch @MON
				Case 10
					$iImage = 5
				Case 12
...
Func __Logging_ValidateSuccess($sMessage)
	...
	Local $sSuccessStrings = "success|Response Received|Successfully|OK!|OK|Registration succeeded|Initiated|Clean" & _
...
Func __Logging_ValidateWarning($sMessage)
	...
	Local $sWarningStrings = "Access is denied|No operation can be performed|rate limited|blocked|" & _

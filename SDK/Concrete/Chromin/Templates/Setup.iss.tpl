;* Chromin - Installer script (Template)
...
;*   %{APP_NAME}       -> Environment["Name"]          (e.g., "Chromin")
;*   %{APP_SHORTNAME}  -> Environment["ShortName"]     (e.g., "Chromin")
;*   %{DISTRO_NAME}    -> Environment["DistributionName"] (e.g., "Chromin_8535")
;*
...
#define ini_DefaultBrowserPath "C:\Program Files\Google\Chrome\Application\chrome.exe"
...
begin
  ChromePath := '';
  // First try to get the Chrome path from the registry (64-bit)
  if RegQueryStringValue(HKLM64, 'SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\chrome.exe', '', ChromePath) then
  begin
    Result := ChromePath;
    Exit;
  end;
  ...
  if FileExists(ExpandConstant('{commonpf}\Google\Chrome\Application\chrome.exe')) then
  begin
    Result := ExpandConstant('{commonpf}\Google\Chrome\Application\chrome.exe');
    Exit;
  end;
  
  if FileExists(ExpandConstant('{commonpf32}\Google\Chrome\Application\chrome.exe')) then
  begin
    Result := ExpandConstant('{commonpf32}\Google\Chrome\Application\chrome.exe');
    Exit;
  end;
  
  // Default to standard Chrome path if nothing else works
...
begin
  ...
  SetIniString('{#app_shortname}', 'BrowserPath', ChromePath, IniFile);
...
begin
  ...
  begin
    ...
    // Only write BrowserPath if we found Chrome or user wants to reset settings
...
begin
  ...
  begin
    // Write the Chrome path to INI file
    WriteChromePath();

...
; Aut2Exe Settings
;===============================================================================================================
#AutoIt3Wrapper_Icon=..\..\Resources\Icons\Ownership.ico		;~ Filename of the Ico file to use for the compiled exe
#AutoIt3Wrapper_OutFile_Type=exe								;~ exe=Standalone executable (Default); a3x=Tokenised AutoIt3 code file
#AutoIt3Wrapper_OutFile=..\..\..\Resolute\Ownership.exe			;~ Target exe/a3x filename.
#AutoIt3Wrapper_OutFile_X64=..\..\..\Resolute\Ownership_X64.exe	;~ Target exe filename for X64 compile.
...
;===============================================================================================================
#AutoIt3Wrapper_Res_Comment=Take Ownership						;~ Comment field
#AutoIt3Wrapper_Res_Description=Take Ownership					;~ Description field
#AutoIt3Wrapper_Res_Fileversion=11.1.1.870
...
#AutoIt3Wrapper_Res_Field=ProductName|Take Ownership
...
; Remove resources
#AutoIt3Wrapper_Res_Icon_Add=..\..\Resources\Icons\OwnershipH.ico					; 201
...
Global $g_sProgShortName		= "Ownership"
Global $g_sProgShortName_X64	= $g_sProgShortName & "_X64"
Global $g_sProgName				= "Take Ownership"
...
Global $g_sUrlProgPage			= "https://www.rizonesoft.com/downloads/ownership/|www.rizonesoft.com/downloads/ownership/"
...
Global $g_hChkInstallPause, $g_hBtnInstall, $g_bInstalled
...
Func _StartCoreGui()
	...
	$g_hSubHeading = GUICtrlCreateLabel(StringFormat($g_aLangCustom[0], $g_aLangCustom[3]), $g_iSizeIcon + 22, 43,  $g_iCoreGuiWidth - 150, 70)
	GUICtrlSetFont($g_hSubHeading, 9)
	GUICtrlSetColor($g_hSubHeading, 0x353535)

	$g_hChkInstallPause = GUICtrlCreateCheckbox(Chr(32) & $g_aLangCustom[2], $g_iSizeIcon + 22, 100, 200, 20)
	GUICtrlSetFont($g_hChkInstallPause, 9)
	$g_hBtnInstall = GUICtrlCreateButton(StringFormat($g_aLangCustom[3], $g_sProgShortName), $g_iCoreGuiWidth - 250, 150, 230, 35, $BS_DEFPUSHBUTTON)
	GUICtrlSetFont($g_hBtnInstall, 9)
	_GetOwnershipShellStatus()
	GUICtrlSetOnEvent($g_hBtnInstall, "_InstallTakeOwnership")
	...
	$g_hIconDonate = GUICtrlCreateIcon($g_aDonateIcons[0], $g_iAboutIconStart, 10, $g_iCoreGuiHeight - 90, 64, 64)
	GUICtrlSetCursor($g_hIconDonate, 0)
	GUICtrlSetResizing($g_hIconDonate, BitOR($GUI_DOCKBOTTOM, $GUI_DOCKLEFT, $GUI_DOCKSIZE))
	$g_hLblDonate = GUICtrlCreateLabel($g_aLangDonate[2], 85, $g_iCoreGuiHeight - 69, $g_iCoreGuiWidth - 120, 32)
...
Func _GetOwnershipShellStatus()
	...
	If RegRead("HKEY_CLASSES_ROOT\*\shell\runas", "") Or _
		...
		GUICtrlSetData($g_hSubHeading, StringFormat($g_aLangCustom[6], $g_sProgName, $g_sProgName))
		GuiCtrlSetState($g_hChkInstallPause, $GUI_DISABLE)
		GUICtrlSetData($g_hBtnInstall, StringFormat($g_aLangCustom[4], $g_sProgName))
		GUICtrlSetColor($g_hSubHeading, 0x1F8522)
		$g_bInstalled = true
	Else

		GUICtrlSetData($g_hSubHeading, StringFormat($g_aLangCustom[5], $g_sProgName, $g_sProgName))
		GuiCtrlSetState($g_hChkInstallPause, $GUI_ENABLE)
		GUICtrlSetData($g_hBtnInstall, StringFormat($g_aLangCustom[3], $g_sProgName))
		GUICtrlSetColor($g_hSubHeading, 0x353535)
		$g_bInstalled = false
...
Func _InstallTakeOwnership()
	...
	If $g_bInstalled = false Then

		If GUICtrlRead($g_hChkInstallPause) = $GUI_CHECKED Then
		...
		_Registry_Delete("HKEY_CLASSES_ROOT\Directory\shell\runas")
		_Registry_Write("HKEY_CLASSES_ROOT\Directory\shell\runas", "", "REG_SZ", "Take Ownership")
		_Registry_Write("HKEY_CLASSES_ROOT\Directory\shell\runas", "HasLUAShield", "REG_SZ", "")
		_Registry_Write("HKEY_CLASSES_ROOT\Directory\shell\runas", "NoWorkingDirectory", "REG_SZ", "")
		_Registry_Write("HKEY_CLASSES_ROOT\Directory\shell\runas\command", "", "REG_SZ", _
			"cmd.exe /c takeown /f " & Chr(34) & "%1" & Chr(34) & " /r /d y && icacls " & Chr(34) & "%1" & Chr(34) & " /grant administrators:F /t" & $WithPause)
		_Registry_Write("HKEY_CLASSES_ROOT\Directory\shell\runas\command", "IsolatedCommand", "REG_SZ", _
			"cmd.exe /c takeown /f " & Chr(34) & "%1" & Chr(34) & " /r /d y && icacls " & Chr(34) & "%1" & Chr(34) & " /grant administrators:F /t" & $WithPause)
		_Registry_Delete("HKEY_CLASSES_ROOT\Drive\shell\runas")
		_Registry_Write("HKEY_CLASSES_ROOT\Drive\shell\runas", "", "REG_SZ", "Take Ownership")
		_Registry_Write("HKEY_CLASSES_ROOT\Drive\shell\runas", "HasLUAShield", "REG_SZ", "")
		_Registry_Write("HKEY_CLASSES_ROOT\Drive\shell\runas", "NoWorkingDirectory", "REG_SZ", "")
		_Registry_Write("HKEY_CLASSES_ROOT\Drive\shell\runas\command", "", "REG_SZ", _
			"cmd.exe /c takeown /f " & Chr(34) & "%1" & Chr(34) & " /r /d y && icacls " & Chr(34) & "%1" & Chr(34) & " /grant administrators:F /t" & $WithPause)
		_Registry_Write("HKEY_CLASSES_ROOT\Drive\shell\runas\command", "IsolatedCommand", "REG_SZ", _
			"cmd.exe /c takeown /f " & Chr(34) & "%1" & Chr(34) & " /r /d y && icacls " & Chr(34) & "%1" & Chr(34) & " /grant administrators:F /t" & $WithPause)
		$g_bInstalled = true
	...
	Else
		...
		_Logging_EditWrite($g_aLangMessages2[5])
		_Registry_Delete("HKEY_CLASSES_ROOT\Directory\shell\runas")
		_Registry_Delete("HKEY_CLASSES_ROOT\Drive\shell\runas")
		$g_bInstalled = false

...
#AutoIt3Wrapper_Res_Fileversion=21.0.0.84
...
Func _ExecuteTool($sExe, $sName, $sParameters)

	Local $sx64Exe = $STR_BINx64 & $sExe
	Local $sx86Exe = $STR_BINx86 & $sExe

	If @OSArch = "X64" Then
		If FileExists($sx64Exe) Then
			_ShellExecuteEx($sx64Exe, $sName, $sParameters)
			Return
		Else
			If FileExists($sx86Exe) Then
				_ShellExecuteEx($sx86Exe, $sName, $sParameters)
				Return
			EndIf
		EndIf
	Else

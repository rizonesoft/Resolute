;* Watermin - Installer script (Template)
...
;*   %{APP_NAME}       -> Environment["Name"]          (e.g., "Watermin")
;*   %{APP_SHORTNAME}  -> Environment["ShortName"]     (e.g., "Watermin")
;*   %{DISTRO_NAME}    -> Environment["DistributionName"] (e.g., "Watermin_8535")
;*
...
#define ini_DefaultBrowserPath "C:\Program Files\Google\Waterfox\Application\chrome.exe"
...
// Add a function to detect Waterfox installation path
function GetWaterfoxPath(): String;
var
  WaterfoxPath: String;
begin
  WaterfoxPath := '';
  // First try to get the Waterfox path from the registry (64-bit)
  if RegQueryStringValue(HKLM64, 'SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\chrome.exe', '', WaterfoxPath) then
  begin
    Result := WaterfoxPath;
    Exit;
  end;
  ...
  if RegQueryStringValue(HKLM, 'SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\chrome.exe', '', WaterfoxPath) then
  begin
    Result := WaterfoxPath;
    Exit;
  end;
  ...
  if FileExists(ExpandConstant('{commonpf}\Google\Waterfox\Application\chrome.exe')) then
  begin
    Result := ExpandConstant('{commonpf}\Google\Waterfox\Application\chrome.exe');
    Exit;
  end;
  
  if FileExists(ExpandConstant('{commonpf32}\Google\Waterfox\Application\chrome.exe')) then
  begin
    Result := ExpandConstant('{commonpf32}\Google\Waterfox\Application\chrome.exe');
    Exit;
  end;
  
  // Default to standard Waterfox path if nothing else works
  Result := 'C:\Program Files\Google\Waterfox\Application\chrome.exe';
end;
...
begin
  // Get Waterfox path
  WaterfoxPath := GetWaterfoxPath();
  
  // Write main settings
  SetIniString('{#app_shortname}', 'FirstRun', '{#ini_FirstRun}', IniFile);
  SetIniString('{#app_shortname}', 'BrowserPath', WaterfoxPath, IniFile);
...
begin
  ...
  begin
    ...
    // Only write BrowserPath if we found Waterfox or user wants to reset settings
...
begin
  ...
  begin
    // Write the Waterfox path to INI file
    WriteWaterfoxPath();

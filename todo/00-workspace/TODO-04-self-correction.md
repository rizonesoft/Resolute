...
- [x] Require base and head on every candidate manifest, and reject a manifest that floats free. Done when: a candidate review without both fails closed, quoted. Done: `assert_candidate_identity` raises on any diff-titled chunk without both; the fence CLI fails closed as `fence: refusing candidate manifest without base and head (diff in: CANDIDATE DIFF; pass --base/--head)` (exit 1). Plan manifests (no diff chunks) need neither; an empty side counts as missing. Self-test: `identity-diff-needs-base-head`, `identity-diff-needs-head`, `identity-empty-base`, `identity-ok`.

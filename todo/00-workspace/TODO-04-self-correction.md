...
- [x] Record cost and latency at record time: the review skill writes measured cost in tokens and latency on every new round line, so the §8 value-per-cost comparison stops reading `unresolved`. Done when: the skill carries the recording step, and a round line written by following it resolves both fields, quoted from a driven record. Done: the skill carries "Record the round" (time each invocation; codex `tokens used` from the round's stderr, comma stripped); driven: `exit=0 latency=17s` with `tokens used / 7,435`, recording as `cost: 7435tokens latency: 17s`.

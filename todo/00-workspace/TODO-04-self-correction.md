...
- [ ] Resolve the candidate mechanically: OID exists, tree matches the reviewed tree, before any stamp is accepted. Done when: an unresolving or mismatched candidate fails closed, quoted. The attest path resolves too: base, head, and tree must resolve and the tree must belong to the head (independent-D00-T04-s12-2026-09-19 P2; §12 pins the args to the manifest's pair, this item resolves them).

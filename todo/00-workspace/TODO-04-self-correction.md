...
- [x] Bind every report to its commit and timestamp, and expose a versioned machine export §8 consumes directly. Done when: `--report` prints its as-of, the export has a version the checker asserts, and §8 reads the export rather than the prose. Done: `--report` opens with `as-of: 0a24c0342660c5e2ab2466e5d674419bfc9305ff 2026-09-19T08:27:54Z`; `--export` emits `export_version: 1` with the as-of binding, `--check-export` asserts it (`export version 1, 24 runs, internally sound`), and a tampered copy fails as `export_version is 99, this checker asserts 1`; §8 now decides `from the run-record export, not the prose` with the version asserted by `--check-export`.

...
**Test checkpoint:** The dropped-commit chunk fails quoted and the contiguous chunk passes; the truncated stamp review fails quoted and `STAMP HOLDS` passes; the poisoned manifest parses the true pair quoted; the unbound checker fails quoted; the replaced findings file fails read-back quoted; the replacement drill reads the real HEAD quoted; all six self-tests pin their legs.

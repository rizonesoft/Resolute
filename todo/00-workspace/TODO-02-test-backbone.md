...
- [x] Drive timer completion to its endpoint: `AnimateToggle` lands on the target palette, content-view hide and error settle, collapse widths settle. Done when: each completion is asserted after a pumped loop, and a stuck transition fails the run rather than hanging it. **Done 2026-09-26:** `tests/ui_driven_test.cpp` (tag `[ui][driven]`) creates a hidden host window, starts the manager's real timer on it, and pumps messages through `PumpUntil`, which returns false past its deadline so the case fails by name. `Theme toggle crossfade settles on the target palette` reads the target palette independently (one instant `Toggle`, two more to complete the cycle) and asserts the crossfade ends on it with `IsTransitioning()` false; `Content view hide and error settle` asserts the empty state's fade-out clears `IsEmpty()` and restores the window's `WS_EX_TRANSPARENT` (read back with `GetWindowLongPtrW`), and that an error's slide, auto-dismiss timer, and fade restore it too; `Sidebar collapse widths settle both ways` asserts `ScaledWidth()` settles at 50 collapsed and 200 expanded at 96 DPI. A deliberately stuck crossfade (duration 1.0e7) failed the case by name, `transition: the theme crossfade`, exit 42, recorded in docs/phase-runs/2026-09-25-phase-0.md and reverted.
- [x] Drive core interactions on window-backed controls: hit-testing, scroll, and column resize on the list view. Done when: each is asserted against a real window and the run is green headful. **Done 2026-09-26:** `List view hit-tests scrolls and resizes columns in a real window` sends real mouse messages to a created (hidden) `ListView`: a click in the fifth visible row selects index 4; one wheel notch scrolls three rows, after which the same first-row click selects index 3; and dragging the first column's edge from 160 to 200 moves a header click at x = 180 from sorting column 1 to sorting column 0. The whole driven set read `All tests passed (40 assertions in 5 test cases)` at the SHIP and `50 assertions in 5 test cases` at the final candidate, after the review's regression legs; both presets `100% tests passed out of 74`.
- [x] Tear a control down with its animations still pending: D00 T02 §5 found manager-held callbacks outliving control teardown, and its test guards drain them first, so no test shows production teardown is safe (plan review PR4 of the D00 T02 §6 review; needs D00 T02 §6 shipped). Done when: a driven case destroys a control mid-animation, pumps the manager timer past the animation's end, and asserts no callback runs against the destroyed control (or that teardown cancels it), and a deliberately leaked callback fails the case, quoted. **Done 2026-09-26:** the test found the defect it was filed for: no control cancelled its animations, so every callback captured a pointer that teardown freed, and the manager's tick also let a callback push into the vector `remove_if` was iterating. The fix is in the shared library: each animation carries its owner (`AnimateFor(owner, ...)`, `AnimateStaggeredFor`), every control call site passes the object its callbacks reach (`this`, or `self`, `lv`, `sb` in static procs and loops), each of `ContentView`, `ListView`, `Sidebar`, `StatusBar`, and `Toolbar` gains a destructor that calls `CancelOwner(this)`, and `OnTick` ticks a moved-out copy while recording cancellations, so an owner cancelled mid-frame never runs again that frame and a callback that starts an animation is safe. `A control torn down mid-animation leaves no callback behind` asserts the manager holds nothing after a mid-collapse teardown, that a probe owner cancelled from another callback in the same frame never runs, and that an animation started from a completion callback runs to its end. With the `Sidebar` destructor's `CancelOwner` removed, the case failed on `mgr.Count() == 0` (`1 == 0`), exit 42, recorded and reverted. **Corrected 2026-09-26:** this item read as a test only; the test cannot pass without the library change, which is in scope because the item asks that teardown be safe, not merely checked. **Corrected 2026-09-26 by the independent review of `c8a31bd3` (two P1s):** an animation already kept earlier in a frame survived a later callback's cancellation of its owner, and a cancellation from an animation's own final update still let its completion run; `Animation::Step` now advances the clock alone, the manager runs the update and the completion itself with a cancellation check between them, and the kept list is filtered against the frame's final cancellations. Two legs in the teardown case pin both; against the SHIP library they failed `earlyRuns == 1` (`77 == 1`) and `completeRuns == 0` (`1 == 0`), recorded in the run file. **Corrected 2026-09-26 by panel round 1:** a callback that pumps a nested message loop could let a nested tick clear the frame's cancellations, so `OnTick` now skips a nested tick; and the teardown leg only waited on `Settled()`, which returns at once after cancellation, so it now creates a real sidebar child, counts the `WM_SIZE` messages its collapse sends the host, and pumps a fixed 400 ms past the 250 ms collapse after teardown, asserting the count never grows. Drills recorded in the run file: without the guard the nested leg failed `nestedRuns == 1` (`77 == 1`); without the `Sidebar` destructor's cancel the teardown leg failed `mgr.Count() == 0` (`7 == 0`).
- [x] Commit: `"test: driven ui completion tests"`
...
- [x] Assert every manifest entry resolves to SVG and to a bitmap. Done when: all resolve, and a deliberately removed icon fails by name, quoted. **Done 2026-09-26:** `tests/icon_manifest_test.cpp` asserts each name resolves through `GetSvgData` (starting `<svg`) and `CreateBitmap`, and that each referrer line still quotes the name; `All tests passed (137 assertions in 1 test case)`. The audit found seven referenced names the set never carried (`inbox`, `alert-triangle`, `check-circle`, `circle`, `loader-2`, `check`, `ellipsis`), each drawing nothing: the content view's empty and error states, the status bar's notification icons, and the toolbar's overflow. They are added from Lucide 1.48.0 under the names the controls use, with their sources in `shared/lucide/icons/SOURCES.md`. With those seven files moved aside, the case failed 14 assertions, two per name, each naming its icon (exit 42), recorded in the run file and restored. **Corrected 2026-09-26:** the item reads as an audit only; making the referenced names resolve is in scope because the item's Done-when is that all resolve. **Corrected 2026-09-26 by panel round 1:** the case checked only manifest to source, so an unlisted misspelled reference passed; `Every icon-shaped literal in the sources is in the manifest` now scans every quoted kebab-case literal in the scoped sources and requires each listed for its file, as an icon referrer or as a declared non-icon (`!` lines, 13 of them, each with its reason). Referrers are files rather than lines, so an unrelated edit that moves a line never breaks the audit. Drills recorded in the run file: a misspelled manifest entry failed on the source's unlisted `"chevron-up"`, and an unlisted `"chevron-upp"` added to `listview.cpp` failed naming it and its line; the `[icons]` set reads `All tests passed (200 assertions in 5 test cases)`. **Corrected 2026-09-26 by panel round 2:** the scan matched only well-formed kebab-case, so `"chevron-Up"` or `"chevron_up"` passed unseen; it now matches any single-token narrow literal (letters, digits, `-`, `_`, any case, wide and UTF literals skipped), with seven more declared non-icons (six `GetProcAddress` entry points and `currentColor`). Drills: each malformed name, added to `listview.cpp`, failed naming it; `[icons]` reads `All tests passed (205 assertions in 5 test cases)`.
...
- [x] Render every shared control offscreen (D2D bitmap target through WIC, no window) in light and dark at 100 and 150 percent, and commit the outputs as golden images under `tests/golden/`, named by control, appearance, and DPI. Done when: every control in `shared/resolute-ui/src/controls/` has four goldens, and a deliberately shifted layout fails the diff naming the control. **Done 2026-09-26:** `tests/ui_render_test.cpp` (tag `[ui][render]`) renders each control through a new `RenderTo` (the five D2D controls swap an offscreen target in for their window's, releasing device-bound icon bitmaps and the shared SVG documents around it; the GDI popup gains `PopupMenu::Measure` and `RenderTo(HDC)` over the one paint path its window uses) into a software WIC bitmap target at 96 DIPs per inch with grayscale text, after `AnimationManager::Flush` runs every animation to its end. 48 goldens under `tests/golden/` (`README.md` there gives naming, making, tolerance, restaking, and host): content view (empty, inbox, error, success), list view, sidebar (expanded, collapsed), status bar with its notification icons, toolbar (full, overflowed), popup menu, and the glyph sheet, each light and dark at 96 and 144. Every golden was looked at before committing. Rendering them found a defect: a toolbar too narrow for its first item hid every command with no overflow button (`m_overflowStart > 0` in three places); one `OverflowRect` now serves paint, hit-test, and the menu anchor, and `toolbar-overflow-*` shows the button. **Corrected 2026-09-26 by the independent review of `bba2c4ad`:** the newly reachable overflow menu offered the View dropdown as a plain command nothing handles; `Toolbar::BuildOverflowMenu` now gives a dropdown its choices as a submenu, pinned by `An overflowed toolbar offers every hidden command`. A one-pixel layout drill (list view row height 28 to 29) failed all four list view goldens by name, 13461 to 29111 differing pixels, recorded in the run file and reverted.
- [x] Pixel-diff the renders against the goldens under ctest with a stated per-pixel tolerance, saving the diff image and the differing-pixel count beside the run on failure. Done when: a green run quotes zero differences, a one-pixel shift fails naming the control with its count quoted, and the tolerance value is recorded in the test with its reason. **Done 2026-09-26:** `CheckGolden` diffs per channel at `kChannelTolerance = 8` (its reason in the source: anti-aliasing rounding across D2D rasterizer builds; a shift moves edges by tens to hundreds of levels), and on a difference writes `<name>-actual.png` and `<name>-diff.png` under `build/<preset>/render-out/`; a green run is `All tests passed (482 assertions in 9 test cases)`, every golden at zero differing pixels. **Corrected 2026-09-26:** review added four cases (the overflow menu, the direct icon API, `RenderTo`'s result, and a golden of another size), and the green run at the review's last answering commit is `All tests passed (519 assertions in 13 test cases)`, re-measured there (panel rounds 2 to 4 of this section's review: the 482 figure predates them). A golden of another size now fails through the same path, with the count over the union of both sizes and the actual and diff images saved, instead of aborting before them. `The golden diff catches a one-pixel shift` keeps the comparator honest on every run: the sidebar's render moved one pixel differs from its golden by 2953 pixels. **Corrected 2026-09-26 by the independent review:** that leg compared pixels before checking sizes; it now requires equal sizes first, and `DiffCount` refuses a mismatch rather than reading past a buffer.
- [x] Extend the runs/ capture convention with the matrix: every section that ships a user-visible surface owes light-by-dark by 100-by-150 captures under `docs/captures/runs/`, named `<date>-<section>-<surface>-<mode>-<dpi>.png`, reusing the §3 sidecar with appearance added, and each shipped surface extends the goldens with its own renders. Done when: the matrix naming is written in `docs/captures/runs/README.md` beside the existing convention and the launcher's four captures plus sidecars are committed as the first instance. **Corrected 2026-09-26:** Done when is now the matrix naming written in `docs/captures/runs/README.md` and the sidecar recording appearance; the launcher's four captures are owed by D00 T02 §10 (panel round 1 of this section's review: the item and the checkpoint still demanded them after they moved). **Done 2026-09-26:** `docs/captures/runs/README.md` gains the appearance-by-DPI matrix (the naming, the sidecar's `appearance` and `monitor dpi` lines, and each surface extending the goldens), and `scripts/capture-window.ps1`'s sidecar now records `appearance`. **Corrected 2026-09-26 by the independent review:** the system setting can disagree with an application that overrides it, and under strict mode an absent registry value threw before the fallback, losing the sidecar; the script takes `-Appearance light|dark` (stated by the caller) and otherwise records the system setting labelled as such, read without throwing. **Corrected 2026-09-26:** the launcher's four captures move to D00 T02 §10, which owns every headful run: a window capture needs the foreground on a desktop the operator is using, and the four need the system appearance changed between runs, neither of which this section may do; the offscreen goldens need neither.
- [x] Make a missing glyph impossible to miss: D00 T02 §8's audit finds icon references by the spelling of a literal, so a malformed name the scan does not match (`"chevron--up"`, `"chevron up"`), a computed name, or an icon use hidden behind a file-level non-icon exemption still resolves to null and draws nothing, and the resolver stays silent when it does (the D00 T02 §8 sign-off leftover, F3, and plan review PR1, PR2, PR4, and PR6 of the D00 T02 §8 review; needs D00 T02 §8 shipped). Done when: every icon name is validated where it is used (the arguments of `LucideIcons::Render`, `CreateBitmap`, `GetSvgData`, and the icon drawing call, and the icon tables of the controls) whatever its spelling, the resolver logs an unknown name once and draws a visible fallback glyph, the goldens show each of the seven glyphs D00 T02 §8 restored at its real size in both appearances, and a malformed name fails by name, quoted. **Done 2026-09-26:** `LucideIcons::Resolve` returns a name that resolves, and otherwise logs it once (`OutputDebugString`), remembers it, and returns the fallback `square-dashed` (Lucide 1.48.0, in `SOURCES.md`); `RenderContext::DrawSvgIcon` resolves every name it draws, and the sidebar, toolbar, and popup resolve theirs before `Render` and `CreateBitmap`, so a malformed name, whatever its spelling or source, draws a visible placeholder. Each render case asserts `UnknownNames()` is empty, capturing the names. With `"system"` renamed `"systemm"` in the sidebar table, the sidebar case failed `unknown := { "systemm" }` and its goldens by 152 to 313 pixels (recorded, reverted). `glyphs-*` shows the seven restored glyphs and the fallback at their drawn size in both appearances. `An unknown icon name draws the fallback and is reported once` pins the resolver.
- [x] Commit: `"test: rendered-output regression tests"`

**Test checkpoint:** `ctest --preset debug -L render` exits 0 with zero differences quoted; a one-pixel shift of one control fails naming the control with its differing-pixel count quoted; `docs/captures/runs/README.md` states the matrix convention. **Corrected 2026-09-26:** it said `docs/captures/runs/` also holds the launcher matrix (four PNG with sidecars); those captures moved to D00 T02 §10, which owns every headful run. The goldens prove composition, the matrix proves a human looked, and the UIA tree keeps the wiring half.
...
-> SOURCE: panel-D00-T02-s8-2026-09-26-F3 D00-T02-S8-F3
...
- [x] Audit every focus-dependent site in `tests/` (window creation, foreground assertions, `capture-window.ps1` invocations, synthetic input if any) into a committed table with a disposition each: convert to offscreen or message-loop-only, fence as headful, or keep with a reason. Every fenced site additionally declares its placement intent: which monitor, which DPI, and why that surface needs that screen (DPI-100 rendering, DPI-150 rendering, cross-monitor move, absolute positioning). Done when: the table quotes every site with its disposition and zero sites are unaccounted, and every fenced site carries a placement intent a second reader can challenge. **Done 2026-09-26:** `tests/focus-audit.md` lists every site in two tables: the test side (the shared hidden host, the driven and coverage clicks, the offscreen renders, the window-less controls, the census probe, the popup, the toolbar's overflow menu, and the four launcher captures) and the library sites the suite can reach (each `SetFocus`, `SetCapture`, `TrackPopupMenu`, the sidebar's tooltip, and the popup's modal loop), each with a disposition; every fenced row carries its `[place:...]` intent and the reason. The audit found no synthetic input (`SendInput`, `keybd_event`, and `mouse_event` appear nowhere in `tests/`) and no test that invoked `capture-window.ps1` before this section.
- [x] Convertibles move to forms that need no foreground (offscreen targets, pumped loops without visible windows) behind shared helpers, and the converted tests stay green. Done when: the default label-filtered run passes with zero focus-dependent calls outside the fenced set. **Done 2026-09-26:** one shared host, `tests/ui_host.h` (`HiddenHost`, `PumpUntil`, `PumpFor`, `Settled`, `Click`), replaces the private hosts in `ui_driven_test.cpp` and `ui_render_test.cpp`; it is hidden and disabled. The guard's first run found the conversion this needed: the list view's `SetFocus` on a click activated the hidden host, which took the foreground (`FOCUS-VIOLATION List view hit-tests scrolls and resizes columns in a real window: took the foreground ... class=ResoluteDrivenHost visible=0`). `WS_EX_NOACTIVATE` and `LockSetForegroundWindow` each failed to stop it; a disabled host does. `ctest --preset debug`: `100% tests passed out of 98`, every census clean.
- [x] True-headful tests (whose point is a visible window: driven completion, foreground captures, per-monitor DPI rendering, cross-monitor moves) are fenced behind a label excluded from the default run and runnable visibly on demand, each running on its declared monitor and DPI from the audit. Done when: the default run activates no window (proven by a foreground log) and the fenced set passes visibly with every window on its declared screen. **Done 2026-09-26:** eight cases in `tests/ui_headful_test.cpp` carry `[headful]` and a `[place:...]` tag: the popup's keyboard choice, Escape, and owner teardown, the toolbar's real overflow button and its system menu, and the launcher at light and dark by 100 and 150 percent. The default presets exclude the label (`"filter": {"exclude": {"label": "headful"}}`); `headful` and `headful-visible` include only it. The default run activates no window: every case's census reads `shown=0 foreground=0 windows=0 violations=0`. The fence's own headful pass ran collecting on the operator's idle, unlocked session (`RESOLUTE_IDLE_COLLECT=1 ctest --preset headful`): `100% tests passed out of 8`, every window on its declared screen, no violation. The pass found four faults the plan did not predict, each fixed here: a failed assertion left four launchers open (an owner now closes the launcher however the case ends); `SetForegroundWindow` from a background process is refused (the case attaches its input to the foreground thread first); the launcher first appeared on the primary before being moved (it now starts at its declared monitor through `STARTF_USEPOSITION`); and the modal system menu dispatches no thread timer (a worker thread drives it). **Re-run 2026-09-26 at `887121c1`**, after the review's changes to the guard, the gate, and the capture path (idle 612 s, unlocked): `100% tests passed out of 8`, `fence-debt: 8 fenced case(s): 8 green, 0 night-owed, 0 unaccounted`, every `CENSUS` line `violations=0`, the launcher shown at `monitor=\\.\DISPLAY2 dpi=96` and `monitor=\\.\DISPLAY1 dpi=144` as declared; the published captures are this pass's.
- [x] Window census joins the gate: the foreground proof records every test HWND with monitor, rect, iconic state, and the monitor's measured DPI, and the gate asserts two things: the default run shows zero visible test windows anywhere plus zero foreground holds, and the fenced run shows every window on its declared monitor and DPI. Done when: a full default run log is quoted clean on the absence half and a fenced run log is quoted clean on the placement half, with one deliberate misplacement failing the gate by name. **Done 2026-09-26:** `tests/focus_guard.cpp`: out-of-context WinEvent hooks for `EVENT_SYSTEM_FOREGROUND` and `EVENT_OBJECT_SHOW` on their own thread, and a census at each case's end of every window the suite owns (this process and any process a case adopts), each with its monitor, rectangle, iconic state, and the monitor's measured DPI (`GetDpiForMonitor`, the suite per-monitor aware from `tests/main.cpp`), written to `build/<preset>/focus-census/<case>.log` and summarised in one `CENSUS` line. Absence half: the default run's 98 cases all read `violations=0`. Placement half: the collecting pass's 8 cases all read `violations=0`, with the launcher windows at `monitor=\\.\DISPLAY2 dpi=96` and `monitor=\\.\DISPLAY1 dpi=144` as declared. The misplacement drill (the Escape popup declared `[place:dpi96]`) failed by name: `FOCUS-VIOLATION The popup menu dismissed with Escape returns no command: a window is not where [place:dpi96] declares: ... class=ResolutePopupMenu visible=1 ... monitor=\\.\DISPLAY1 dpi=144`. A violation exits 5 even when every assertion passed.
- [x] `docs/testing.md` documents the uninterrupted gate: the exact default-run command, the fenced on-demand command, and what green means for each. Done when: a second section can follow it without asking. **Done 2026-09-26:** `docs/testing.md` states the three runs (`ctest --preset debug`/`release`, `ctest --preset headful`, `ctest --preset headful-visible`) with what green means for each, the focus guard and its census log, the fence's decision table, the placement tags, and how skips become `Night-owed:` debt through `scripts/fence-debt.py`.
- [x] Fenced headful tests self-skip outside the quiet-hours window (02:00-06:50 local) through one shared gate, so a daytime full run cannot interrupt. Done when: boundary fixtures pin the window math, a daytime fenced run skips every headful test with the window named, the nighttime full run executes them, and the window plus overrides are recorded in `docs/testing.md`. Every skip prints one machine-readable `SKIP <test> <reason>` line, which is the debt list the stamp records rather than a hand-typed copy. The gate honors an idle-collect signal from the §11 runner (session unlocked plus idle past the stated threshold), and collection aborts instantly on input with completed tests recorded and the rest re-queued. **Done 2026-09-26:** `tests/fence.h` (`RESOLUTE_HEADFUL_GATE`, `RESOLUTE_HEADFUL_CHECK_INPUT`) and `tests/fence.cpp`, one gate for every fenced case. Boundary fixtures pin the window (`01:59` out, `02:00` in, `06:49` in, `06:50` out) and the decision (visible beats all, the idle signal collects by day, other values skip). A daytime `ctest --preset headful` skips all eight with lines such as `SKIP "Launcher capture, dark at 100 percent" headful: outside the quiet-hours window 02:00-06:50 local (now 19:53); RESOLUTE_HEADFUL=visible runs it on demand`, and ctest reports them `Skipped` (`SKIP_RETURN_CODE 4`). The collecting path the night window shares ran on the idle signal and executed all eight; the clock trigger itself is pinned by the fixtures, and D00 T02 §11's night run exercises it. The interruption drill moved the pointer one pixel and back mid-case: `SKIP "The popup menu returns the command the keyboard chooses" operator input resumed; re-queued`, census clean. The window, overrides, and SKIP format are in `docs/testing.md`. Panel round 1 of this section's review: a collecting case also needs the operator away at its own start (`fence::kCollectIdleMs`, 120 s), so once the operator returns every later case stands down rather than taking a fresh baseline; live, with the operator at the keyboard, `RESOLUTE_IDLE_COLLECT=1 ctest --preset headful` gave `SKIP "Launcher capture, dark at 100 percent" headful: the operator is active (last input 0 s ago, under the 120 s a collecting case needs); re-queued` for every case.
- [x] `review-todo-section` gains the debt-completeness check: every fenced test the audit table names for the section's surface appears in the green list or the `Night-owed:` list, and a test in neither fails review like an unaccounted control. Done when: the skill carries the check and this section's own review quotes the comparison. **Done 2026-09-26:** the skill's surface check now requires `scripts/fence-debt.py` (with `--self-test` in `check-all`) over the section's fenced run, refusing a case in neither the green nor the `Night-owed:` list, and requires a section that changes the fence to prove it with one headful pass. This section's comparison: the collecting pass reads `fence-debt: 8 fenced case(s): 8 green, 0 night-owed, 0 unaccounted` (`Night-owed: none`); the daytime run reads `8 night-owed`, each with its SKIP reason.
- [x] Settle the fence's contract from the D00 T02 §7 plan review: D00 T02 §7's driven cases run in hidden windows, so the fence classifies each case by what it needs (hidden and background-safe in every default run, visible only where a case truly needs the desktop); a callback or modal loop that never returns hangs ctest, since no test preset sets a timeout; the no-foreground and no-visible-window promises need continuous observation of transient activation, not a poll; initial fence validation must be told apart from sections that defer their headful work, so an unproven fence cannot certify itself through skips; and resumed operator input during a synthetic drag or modal interaction must cancel it and clean up (plan review PR1, PR7, PR11, PR12, and PR13 of the D00 T02 §7 review; needs D00 T02 §7 shipped). Done when: each case carries its visibility class and the default run executes every hidden case, the test presets set a per-test timeout and a deliberately blocked dispatch fails bounded and named, a deliberate brief activation fails the observation, the fence's own validation is one explicit headful pass, and an interruption drill leaves no capture, held input, or window behind, quoted. **Done 2026-09-26:** each case's visibility class is its tier in `tests/focus-audit.md` (every D00 T02 §7 case is default and runs in every default run); the four test presets set a 120 s timeout, and a deliberately blocked dispatch failed bounded and named (`Drill a dispatch that never returns ...***Timeout 120.02 sec`); a deliberate brief show (a 1-pixel window shown without activation) failed `FOCUS-VIOLATION ... showed a window`, and the natural drill, the list view's click, failed `... took the foreground`; the fence's own validation is the collecting headful pass above, not a skip; the interruption drill left no capture, input, or window (census `windows=0`), and a stray window holding the capture failed `... a window was left behind`. The popup's modal loop now ends when its window is destroyed under it and re-posts a `WM_QUIT` it takes; reverted, the owner-teardown case hung until `***Timeout 120.03 sec`.
- [x] Cover the driven surface D00 T02 §7 left: popup selection, dismissal, and teardown; toolbar overflow and filtering through their observable effects; pending-animation teardown of `ContentView`, `ListView`, `StatusBar`, and `Toolbar` with their child windows, as the `Sidebar` case does; rapid reversal (repeated theme toggles, collapse reversals, a replaced error) while animations run; and hit targets and resize boundaries at each supported DPI scale, not only 96 (plan review PR2, PR3, PR4, PR6, and PR10 of the D00 T02 §7 review; needs D00 T02 §7 shipped). Done when: each is a driven case asserted through a read-back, a deliberately broken variant of each fails by name, and the set runs under the fence, quoted. **Corrected 2026-09-26:** it said "toolbar overflow and filtering"; the toolbar has no filter, and the library's only one, the sidebar's, has no input path (`m_searchText` is read by `ApplyFilter` and written nowhere), so it has no observable effect to drive; the launcher's tool-list filter is specified by D03 T02 §3, which owns wiring one. **Done 2026-09-26:** `tests/ui_coverage_test.cpp` (default) and `tests/ui_headful_test.cpp` (fenced): popup selection, Escape, and owner teardown; the toolbar's overflow as a growing set of hidden commands (none at full width), its keyboard path, and its real overflow button choosing Refresh; teardown mid-animation of `ContentView`, `ListView`, `StatusBar`, and `Toolbar`; rapid reversal of the theme crossfade, the sidebar collapse, and a replaced error; and list view and sidebar hit targets and the column resize boundary at 96, 120, 144, and 192. Each broken variant failed by name and was reverted (docs/phase-runs/2026-09-25-phase-0.md): `StatusBar` without its cancel (`c.name := "StatusBar"`), a theme toggle ignored mid-crossfade, a sidebar reversal ignored, a second error leaving the first's slide-in running, an unscaled list view header (`dpi := 192`), and an overflow menu that never builds (`CHECK( previous.count(rui::IDC_TB_REFRESH) == 1 )`). Every case runs under the guard.
- [x] Take the launcher's four captures as the first instance of D00 T02 §9's appearance-by-DPI matrix: light and dark at 100 and 150 percent, named `<date>-<section>-<surface>-<mode>-<dpi>.png` with `scripts/capture-window.ps1`'s sidecar (its `appearance` and `monitor dpi` lines), taken under the fence because a window capture needs the foreground and both appearances (moved from D00 T02 §9, whose offscreen goldens need neither; needs D00 T02 §9 shipped). Done when: the four PNG and their sidecars are committed under `docs/captures/runs/`, each sidecar's appearance and DPI agree with its name, and the run left the desktop as the fence requires, quoted. **Done 2026-09-26:** `docs/captures/runs/2026-09-26-D00-T02-s10-launcher-{light,dark}-{100,150}.png` with sidecars, taken by the four fenced cases through `scripts/capture-window.ps1 -ProcessId` into `build/<preset>/captures/` and published by copying them in after looking at each (panel round 1 of this section's review: a capture case writing into the tracked directory could overwrite or delete committed evidence on a re-run); each sidecar agrees with its name (`monitor dpi : 96` or `144`, `appearance : light` or `dark (stated by the caller)`, `dpi aware : True`), and the launchers closed after each (none running after the pass). The first captures found two faults in the capture script, fixed here: it read virtualized coordinates on the 150% monitor (saved at 733 x 480 and labelled 96 DPI; the script's thread is now per-monitor aware), and it copied the window rectangle, whose invisible resize borders carried a strip of the operator's other windows into the image (it now copies the frame DWM draws). They also show a launcher defect: its window opens at 1100 x 720 physical pixels whatever the DPI, so at 150% two columns are cut off; folded into D01 T02 §2's window-geometry item.
- [x] Commit: `"workspace: convert UI suite to focus-free input"`

**Test checkpoint:** The default `tests/` run passes while the operator's foreground window never changes (foreground log plus census quoted clean); the fenced set passes in a visible on-demand run. Cheaper substitute that fails: running the suite while the operator is away and calling it uninterrupted. **Clarified 2026-09-26:** the visible run is any fenced run whose windows are on the desktop: the operator's on-demand `headful-visible`, or the collecting pass the fence runs itself on an idle, unlocked session, which also stops on input. This section's is the collecting pass; nobody was present to ask for the on-demand form. A section shipping outside the window flips on DAY-green with its `SKIP` lines recorded as `Night-owed:`; nothing waits for 02:00.

-> SOURCE: operator-2026-09-19-visual-timers-s10
-> SOURCE: plan-D00-T02-s7-2026-09-26-PR1 D00-T02-S7-PR1
-> SOURCE: plan-D00-T02-s7-2026-09-26-PR2 D00-T02-S7-PR2
-> SOURCE: plan-D00-T02-s7-2026-09-26-PR3 D00-T02-S7-PR3
-> SOURCE: plan-D00-T02-s7-2026-09-26-PR4 D00-T02-S7-PR4
-> SOURCE: plan-D00-T02-s7-2026-09-26-PR6 D00-T02-S7-PR6
-> SOURCE: plan-D00-T02-s7-2026-09-26-PR7 D00-T02-S7-PR7
-> SOURCE: plan-D00-T02-s7-2026-09-26-PR10 D00-T02-S7-PR10

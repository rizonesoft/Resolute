...
- [ ] Settle how dependencies arrive, given that the tree has none after `D00 T03 §1`. Done when: the decision is dated, names how Catch2 arrives as the only third-party code, and states the rule for adding a dependency later. Cheaper substitute: adding a package manager for a single test framework.
...
- [ ] **Correct the default build, which is not static today.** Measured 2026-09-17 on the merged tree, building the `release` preset: `shared/exo-ui/CMakeLists.txt:33` and `shared/lucide/CMakeLists.txt:58` declare `SHARED` libraries, the application links those, and the shipped set is `Bin/Release/ExoSuite.exe` at 1,380,352 bytes **plus** `System/ExoUI.dll` and `System/Lucide.dll`, totalling 3.9 MB. A `ExoUI_static` target exists at `shared/exo-ui/CMakeLists.txt:27` with an `EXOUI_STATIC` definition, so the static path is available and simply is not the default. Done when: the default preset links the static targets, the shipped set is one executable, and its size is recorded. Cheaper substitute that fails the checkpoint: shipping the DLLs beside the executable and calling the result standalone, which satisfies the letter of per-tool distribution while multiplying what every tool must carry.

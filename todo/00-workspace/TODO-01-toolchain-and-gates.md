...
> **CRUD:** applicable | driven: `scripts/bootstrap.ps1` wrote `reskit/llvm-mingw` from a hash-verified download and its `.pinned-version` stamp was read back by `scripts/cpp-env.ps1`; the destructive path was exercised twice, once by hash mismatch and once by a traversal `dir`, and in both the existing install was still present afterwards
> **Duration:** 335
...
> **CRUD:** applicable | driven: `reskit/Build-Resolute.ps1` (historical: removed by D00 T01 §4, superseded by `scripts/build.ps1`) and `reskit/Build-Extension.ps1` (historical: removed by D00 T01 §4, superseded by `scripts/build.ps1`) were **run**, not inspected, which is how three defects surfaced including one predating this session: PowerShell does not expand `$Var` in the bareword `-DNAME=$Var`, so these scripts had never configured a build type
> **Duration:** 19
> **Implementer:** Claude Opus 5 (claude-opus-5[1m])
...
> **Verified:** 2026-09-17 | §6 | three pins bumped **one at a time with the full gate between each**, per this section's own build order, so a new diagnostic could be attributed: ninja `1.13.1`→`1.13.2`, cmake `4.2.3`→`4.4.3` with no new policy warnings, llvm-mingw `20251216`→`20260908` · every hash taken from a real download and cross-checked against GitHub's own recorded digest, all three matching, byte counts matching · **the date hid the number that mattered: clang 21.1.8 → 23.1.1**, two major versions, read from the installed binaries · **zero new compiler warnings** across a clean rebuild at `-Wall -Wextra -Werror` in both configurations, nothing suppressed and no `-Wno-` added · `clang-tidy` 103→**169**, attributed exactly: +49 `bugprone-signed-bitwise`, +12 `bugprone-unhandled-code-paths`, +4 widened `misc-use-internal-linkage`, +2 `performance-faster-string-find`, **-1** retired, and **every pre-existing check reports an identical count**, which is what makes it a stronger analyser rather than worse code · **the retired finding is the return on currency**: `bugprone-nondeterministic-pointer-iteration-order` at `src/main.cpp:382` no longer fires on unchanged code, the check still exists, and the sort compares cell contents rather than pointer values, so clang-tidy 21 was wrong and 23 is right · binary 2,512,384→**2,744,832**, +9.25 percent, recorded rather than explained away, imports unchanged · `scripts/toolchain-latest.ps1` reports three distinct exit codes, all driven, including a **partial** failure where a confirmed stale pin is still reported while overall currency stays unknown · advisory never automatic, proven by search: zero write calls, the manifest is only ever read · wired into the gate as a warning that does not stop a build, driven · bootstrap from a deleted `reskit/ninja` installs and leaves `git status` clean · driven: window 'Resolute', responding · `check-all` 11 gates ok, 1 not present
> **Review:** round 2, candidate `0227f4b` `d0c8441` -- `adversarial` approve after fixes (2) · `consistency` approve · `integration` approve · `source-defect` approve · `design` approve after fix (1) · `record` approve after fixes (2). Raw findings: docs/reviews/00-workspace/D00-T01-s6.md
> **Independent:** `codex review --commit 0227f4b` (gpt-6-astra, high) returned **one P2 and it was right**, and it is the mirror of the defect this script was written to avoid. The script guards carefully against reporting an unreachable API as a stale pin or as current, and then **discarded a pin it had confirmed stale** when a different lookup failed, because the `exit 2` branch came first. Being careful about one direction of a reporting defect is what made the opposite direction invisible. The reviewer's probe was a **partial** failure, CMake timing out while Ninja returned a newer tag; every probe I wrote was total. Sixth section running where it found something by constructing a state I had not thought to construct.
> **CRUD:** applicable | driven: `toolchain.json` is the data this section writes, and every write was followed by a bootstrap, a locator check and a full gate. The failure paths are exercised directly: a pin lowered to prove the advisory fires without stopping a build, a repository name broken to prove unreachable is distinguishable from behind, and both at once to prove a confirmed finding survives a partial failure. The reverse is one file: restoring the three previous versions and running `bootstrap.ps1 -Replace` returns the exact prior toolchain, because every entry carries its own SHA-256.
> **Duration:** 27
> **Implementer:** Claude Opus 5 (claude-opus-5[1m])
> **Deferred:** the 169 findings stay in the baseline. `D07 T01 §2` ratchets them, and the 49 `bugprone-signed-bitwise` arriving in one bump is now the largest single group in it. -> XREF: D07 T01 §2 -- the ratchet that drives this count down
...
> **Moved:** 2026-09-17 to todo/06-distro-release/TODO-01-build-and-release.md (operator instruction); worked there as `D06 T01 §9` by that file's owner.

**Why it left.** This was written as a toolchain concern and it is a **distribution** one: what it proves is that a machine with nothing installed can build and run the suite, which is the promise the architecture was chosen for rather than a property of the bootstrap. Sitting in Phase 0 it blocked `D00 T02` and `D01 T01`, because both declare a dependency on the whole of `D00 T01`, so a Phase 4 release gate on hardware nobody has was holding the test backbone and the entire framework. It now sits beside `D06 T01 §3`, which needs the same clean machine for install testing.

**What went with it**, so nothing was lost in the move: the six items, the checkpoint, and the evidence gathered on 2026-09-17 that narrows what is left to prove. Three residuals are named there, a registry fallback, a `PATH` dependency, and the unknown one, and the items now report on the first two explicitly.

**What stayed here.** `§1` proves the narrower claim that a compile and link consults nothing outside `reskit/`, and that is unaffected. The environment-poisoning measurement is recorded in `D06 T01 §9` rather than duplicated here.

- [ ] ~~Record what the machine is before anything is installed on it.~~ -> `D06 T01 §9`
- [ ] ~~Clone and bootstrap with nothing else present.~~ -> `D06 T01 §9`
- [ ] ~~Build the real application, not a sample.~~ -> `D06 T01 §9`
- [ ] ~~Run it.~~ -> `D06 T01 §9`
- [ ] ~~Record what the run needed that the bootstrap did not supply, if anything.~~ -> `D06 T01 §9`
- [ ] Commit: ~~`"workspace: the bare-machine proof"`~~ -> `D06 T01 §9`, which commits as `"release: the bare-machine proof"`
...
- [x] Inventory every Linux/WSL-execution surface: `scripts/wsl.sh`, CI jobs on Linux runners, scripts requiring a Linux interpreter, tests that fork Linux tooling, and docs or plan text instructing WSL flows. Done when: the inventory lists each item with its references (what calls or cites it), quoted from grep, and names what stays: Git Bash at `C:\Program Files\Git\bin\bash.exe`, which the review skill requires; the llvm-mingw Windows-targeting toolchain; the stdlib Python tooling that runs on Windows. **Done 2026-09-26:** `git grep -i -E "wsl|ubuntu|linux|python3 "` outside `resolute_au3/`, the review records, the run files, the captures, and the brainstorm found five executable or active items: `scripts/wsl.sh` (cited by `README.md:35-40`, its only caller), the README's WSL flow, `plan-gates` on `ubuntu-24.04` calling `python3` (`.github/workflows/plan.yml:15-28`), the `under WSL and CI` interpreter comment in `scripts/check-all.ps1:78`, and the `WSL checkouts` line in `scripts/review_prompt.py:11`. No test forks Linux tooling and no code path is Linux-only. What stays, each with its reason: Git Bash at `C:\Program Files\Git\bin\bash.exe` (`.claude/skills/review-todo-section/SKILL.md:96` and `scripts/campaign_guard.py:2216` name WSL only to say why the `bash` on PATH must not be used); `detect_context` in `scripts/todo-graph.py:268` and its `display-session fails off Windows` leg (a fail-closed guard, not a Linux path); the workflow parser's model of GitHub's Linux runners in `scripts/review_prompt.py` (it reads what GitHub runs, whatever the runner); the `#!/usr/bin/env python3` shebang of `scripts/profile-check.py` and the `python3` probe in `tools/githooks/pre-commit` (inert or Git Bash, both on Windows); the llvm-mingw toolchain; and the stdlib Python tooling. **Corrected 2026-09-26 by panel round 1:** the inventory and removal results are now quoted in docs/phase-runs/2026-09-25-phase-0.md: the exact command with its exclusions, the 45 hits before and the 15 kept hits after, and the per-name greps for `wsl.sh`, `ubuntu-24.04`, and `python3 scripts`. **Corrected 2026-09-26 by panel round 2:** the full driven output of the broader `wsl|ubuntu|linux|python3 ` inventory at both revisions, and each per-name grep with git's exit code, is committed whole in `docs/captures/runs/2026-09-26-d00-t01-s9-linux-inventory.txt` (`wsl.sh`: exit 1, no match; `ubuntu-24.04` and `python3 scripts`: only `scripts/review_prompt.py`'s workflow-parser fixtures). **Corrected 2026-09-26 by panel round 3:** the first capture cut each line at 240 characters, so it was not whole; it is regenerated with complete matching lines (the only edit is git's leading `<commit>:` prefix), and each grep carries its exit code.
- [x] Remove the bridge and its callers: delete `scripts/wsl.sh` and re-point or delete everything referencing it. Done when: no reference to `wsl.sh` or WSL execution remains outside this section's own record, quoted from grep. **Done 2026-09-26:** `scripts/wsl.sh` is deleted and the README's WSL paragraph, command block, and wrapper note with it; `git grep -n -i wsl` outside the kept records returns only the Git Bash guards, the `detect_context` rationale, this section, its reciprocal XREF in D00 T04 §39, the stamped history of D00 T04 §24 and D00 T04 §39, a D00 T04 §26 item naming a past proof, and the D00 T05 §4 correction note.
- [x] Remove the remaining inventoried surface: Linux-runner CI jobs, Linux-gated tests, Linux-conditional code paths. Done when: the step-1 inventory reads empty item by item, each quoted, and nothing removed has a live caller left behind (grep for each removed name returns nothing outside this section's record). **Done 2026-09-26:** `plan-gates` now runs on `windows-2025` through the runner's Git Bash (`defaults: run: shell: bash`) with `python`, a recorded default: the gates run on the host family they are developed on, at about twice the runner minutes per read-back, and reverting is a two-line edit. **Corrected 2026-09-26 by self-review, before the independent review:** the first version set `shell: bash` as job defaults, which `ci-wait`'s re-run reader does not read, so a red would have printed "cannot reproduce locally: the job sets run defaults" for every step; each step now names `shell: bash`, and its printed template `bash --noprofile --norc -eo pipefail {0}` normalizes to the `shell:` line GitHub logged on the first `windows-2025` run (`C:\Program Files\Git\bin\bash.EXE --noprofile --norc -e -o pipefail {0}`, run 36236461973). The two comments are reworded (`python` usually, `python3` on some installs; CRLF and LF checkouts). No Linux-gated test or Linux-conditional code path existed, so none was removed.
- [x] Prove the Windows tree whole: full gates green plus one review-prompt fence round under Git Bash. Done when: `check-all` reports green quoted, and a `fence`/`cross-check` round runs under Git Bash quoted, proving the purge took no Windows-native tooling with it. **Done 2026-09-26:** `check-all: 20 gate(s) ok` on the purged tree; the Git Bash fence round and the first `windows-2025` `plan-gates` run are recorded in docs/phase-runs/2026-09-25-phase-0.md.
- [x] Settle the review-prompt self-test's POSIX status: under WSL `python3 scripts/review_prompt.py --self-test` fails `anchors-untracked-fires` (a fixture cite written with a Windows separator, `todo\README.md`), found by the D00 T04 §39 independent review and present before it (the pre-§39 suite failed three legs under WSL, the post-§39 suite one). Done when: either the self-test is declared Windows-only with the rest of the Linux surface this section removes (and the AGENTS.md line saying the TODO tooling runs anywhere says so), or the fixture is separator-neutral and the suite passes under a POSIX interpreter, quoted. **Done 2026-09-26:** declared Windows-only with the rest of the tooling: `AGENTS.md` now says the TODO tooling runs on the Windows development host and CI's `windows-2025` runner and nothing tests it elsewhere, replacing "runs anywhere". The separator-bound fixture stays as it is, since no POSIX interpreter runs it.
- [x] Scope the purge to what executes: the removal criteria name every WSL reference outside this section's record, which would rewrite stamped review evidence and historical run notes, and removing Linux-gated paths would take the Linux-runner oracle captures and the portable template checks D00 T04 §37 and D00 T04 §39 rely on (plan review of the D00 T04 §39 candidate, PR16 and PR17; needs D00 T04 §39 shipped). Done when: the inventory separates executable paths and active guidance (removed) from immutable review evidence, historical notes, captured drill logs, and Git Bash dependencies (kept, each with its reason), the supported host and interpreter matrix is decided per script before anything is removed, and the grep criteria exempt the kept records, quoted. **Done 2026-09-26:** the host matrix, decided per script before removal: every `scripts/*.py` and `*.ps1` runs on Windows (the development host and the `windows-2025` runner), under Git Bash where a skill says bash, and no script is supported elsewhere. The inventory above separates what was removed from what stays and why; the kept records (stamped TODO text, `docs/reviews/`, `docs/phase-runs/`, the Linux-runner oracle captures under `docs/captures/ci-oracle/` that D00 T04 §37 and D00 T04 §39 read, and `resolute_au3/`) are exempt from the grep criteria, so nothing historical was rewritten.
- [x] Commit: `"workspace: remove the Linux execution surface"`

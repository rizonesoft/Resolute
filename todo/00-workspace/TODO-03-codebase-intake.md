...
**Adjacency:** list=not-applicable (a codebase intake holds no records a user browses); document=not-applicable (D00 T04 §5, 2026-09-17: this anchored on "Correct the Stale Documentation", which is repository prose. The `document` kind is a user-carried artifact, a PDF or a print or an attachment, and an intake produces none); settings=not-applicable (no user-facing settings are introduced by a move); reporting=not-applicable (nothing here reports to a user); notifications=not-applicable (a one-time intake notifies nobody); permissions=not-applicable (no role model in a source move); audit=applicable @ D00 T03 §1; exchange=not-applicable (nothing is imported or exported at runtime); reverse=not-applicable (git history is the reverse for a source move)

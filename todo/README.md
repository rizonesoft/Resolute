...
| `stamp-no-opus-panel` | FATAL | A stamp dated after the Opus-panel rule landed whose findings carry no panel verdicts. It reads as reviewed evidence while verifying nothing. From 2026-09-23 (D00 T04 §27) the governing record is the last `GPT panel` section; a last `Claude panel` (or legacy `Opus panel`) section is the double-GPT-outage cross-fill and must carry a `GPT outage` line. Earlier stamps keep the Opus-governed rule, where a last `GPT panel` section needs an `Opus outage` line. |

...
|  17   |   §17   | About dialog, from the registry                | §10, D01 T01 §7, D01 T03 §7 |  [ ]   |
|  18   |   §18   | F1 context help through the surface map        | §14, D01 T01 §7 |  [ ]   |
...
- [ ] Produce a portable edition of the whole suite: the launcher plus every tool in one folder that runs from anywhere and writes nothing outside it. Done when: a file-system trace of a suite portable run shows no write outside it, quoted, and the launcher finds every tool with no paths configured.
- [ ] Offer portable extraction from the installer: the generated `.iss` carries a Standard-install versus Portable-extraction choice, and the portable choice lays down the program files plus a starter `<Tool>.ini` with `Portable=1` per the `D01 T01 §2` detection, writing no registry key, no Start menu entry, and no uninstaller. Done when: both choices complete unattended via response file, a trace shows the portable choice writes nothing outside the target folder and no registry key, and the extracted folder runs portable on first launch with no configuration.
...
- [ ] Refuse to publish an unsigned artifact, checked after signing (groom 2026-09-23 gap scan) : profile clause C21 needs a verifying signature on each executable and installer (`docs/conformance-profile.md:54`), but the conformance check runs in check-all before this section signs, so C21 either fails every release or is skipped. Done when: a post-signing gate verifies every executable and installer per tool, refuses to publish naming any unsigned artifact, and `D07 T01 §3` records C21 as a post-signing clause, quoted.
- [ ] Commit: `"release: generated inno setup installers, per tool and whole suite"`

**Test checkpoint:** A per-tool `.iss` and the suite `.iss` are both generated by the release command with no hand editing, and adding a fixture tool produces its installer with no script change. `/SILENT` and `/VERYSILENT` complete unattended and return documented exit codes. A portable run writes nothing outside its folder, traced. A suite portable run writes nothing outside its folder, traced, with every tool found unconfigured. The installer offers Standard and Portable types; the portable extraction writes nothing outside its folder and no registry key, and runs portable unconfigured. Every artifact verifies as signed. Install, run, and uninstall are proven on a named clean virtual machine, with the post-uninstall comparison quoted. The individual-then-suite case is driven and neither duplicates nor orphans.
...
- [ ] Define the update file's version key and channel mapping for old and new clients (groom 2026-09-23 gap scan) : AutoIt clients compare a whole-number `LatestBuild` (live files already sit high, e.g. `resolute_au3/SDK/www/update/Firemin.ru:2` at 8398) while the C++ client compares tag-derived versions (`§11`). Done when: the schema names the version key, sets a `LatestBuild` rule that stays above every AutoIt build so AutoIt users see the upgrade, maps the `§11` stable and preview channels onto the `.ru` and `.ruz` files, and a generated pair of files passes both parsers, quoted.
...
- [ ] Write the bad-release procedure (groom 2026-09-23 gap scan) : clients only notice a higher version (`Update.au3:109`), so restoring an old update file strands everyone who installed the bad build. Done when: the checklist states that a withdrawal is always a re-release at a higher version, that previous installers and update files are retained, and how affected users are told, and a dry run of the procedure on a scratch channel is recorded.
...
- [ ] Record **what Rizonesoft owns and can therefore license at will** (**Groomed 2026-09-23:** the ExoSuite UI library is now `shared/resolute-ui`): ExoSuite and its UI library, RegStudio, and the AutoIt suite are all Rizonesoft copyright. Done when: the policy states that the licence on owned code is a choice rather than a constraint, so `RegStudio` sitting at MIT is a decision to make, not an inconsistency to repair.
...
- [ ] Re-point the README tool gallery at the registry (**Groomed 2026-09-23**, from `D00 T05 §2`): the gallery generator reads names, jobs, versions, and download links from the registry instead of the AutoIt `.sni` files. Done when: the gallery regenerates from the registry clean under its `--check` gate, quoted, with a download link per shipped tool.
...
-> XREF: D06 T01 §17 -- the dialog that renders this registry (moved from D01 T01 §12 on 2026-09-23)
-> XREF: D00 T05 §2 -- the README tool gallery this registry feeds
...
-> XREF: D06 T01 §18 -- the F1 behavior that reads this map (moved from D01 T01 §13 on 2026-09-23)
...
## 17. About Dialog, From the Registry

> **Arrived 2026-09-23 from `D01 T01 §12`**, by operator decision during the 2026-09-23 groom: in framework-core it closed a whole-TODO gate loop (`D04 T01` waits on all of `D01 T01`, this section waits on `D06 T01 §10`, and this file waits on all of `D04 T01`). It keeps its phase-4 place. The code it touches stays under `src/framework/ui/`, owned by the framework layer per `AGENTS.md`; this section edits it there and adds no framework file of its own.


The suite gets one traditional modal About dialog, Help menu > About, owned by the framework and rendered from the `§10` identity registry: no per-tool copy, no pasted string, no settings sidebar panel. Operator-confirmed 2026-09-19: the logo renders at 80px, and every tool window plus the launcher shell gains a Help menu entry. The tool descriptor selects which registry entry a tool shows; the registry holds every string the dialog draws.
...
- [ ] Commit: `"release: about dialog from the identity registry"`
...
-> XREF: D01 T01 §12 -- where this section was written; its Moved marker points here
-> XREF: D01 T01 §7 -- the UI surfaces this dialog builds on

## 18. F1 Context Help

> **Arrived 2026-09-23 from `D01 T01 §13`**, by operator decision during the 2026-09-23 groom: in framework-core it closed a whole-TODO gate loop (`D04 T01` waits on all of `D01 T01`, this section waits on `D06 T01 §10` through `§14`, and this file waits on all of `D04 T01`). It keeps its phase-4 place. The code it touches stays under `src/framework/ui/`, owned by the framework layer per `AGENTS.md`; this section edits it there and adds no framework file of its own.


F1 opens context help: the guide page for the focused surface through the `D06 T01 §14` map, help-home for unmapped surfaces and no-focus. F1 is unbound today (no `VK_F1` handler and no URL launcher anywhere in `src/` or the UI library), and the AutoIt suite has no F1 help either, so this section is a deliberate addition fenced as new behavior: it changes no cloned surface, it adds a key the originals never had. Operator-confirmed 2026-09-19: a Help menu entry ships alongside F1, matching the §17 About pattern of one framework entry in the launcher shell and every tool window.
...
**Chrome Needs:** a Help menu entry per window, following the §17 entries; no other visible control.
...
- [ ] Invoke the default browser with local fallback. Done when: the first real URL launcher seam ships here (the §17 drive mocks it), web-unreachable falls back to the local pages, and no tool carries its own launcher. Cheaper substitute that fails the checkpoint: shelling the URL from each window, which is how fourteen launchers drift apart.
...
- [ ] Commit: `"release: F1 context help through the surface map"`
...
-> XREF: D01 T01 §13 -- where this section was written; its Moved marker points here

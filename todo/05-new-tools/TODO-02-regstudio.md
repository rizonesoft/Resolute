...
> [!IMPORTANT]
> **Current state (verified 2026-09-16):** RegStudio existed twice at the time this was written and both copies were byte-identical in `src/` and `CMakeLists.txt`. **Corrected 2026-09-17:** the duplicate is gone. `862cfb5` removed the ExoSuite copy and `1b99463` took the authoritative one in at `extensions/RegStudio`, so exactly one tree remains. The original wording follows for the record: `samples/RegStudio/` carries the git history and a build output, and `samples/ExoSuite/extensions/RegStudio/` is the copy that arrives with the ExoSuite intake. The authoritative history is the root copy, latest commit `c9b8a0b`.
>

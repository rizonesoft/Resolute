...
- [ ] Route settings by mode: installed resolves under `%LOCALAPPDATA%\Rizonesoft\<Tool>\` (Local, because machine-specific settings must not roam; cost of changing is one constant), portable resolves to `<Tool>.ini` next to the exe, matching the AutoIt locations, with any deviation recorded and reasoned. Done when: both modes resolve to those locations and the Local-over-Roaming choice is recorded with its cost.
- [ ] Detect the mode in a fixed order: a next-to-exe `<Tool>.ini` wins and its `Portable` key decides (absent-or-1 means portable, 0 means installed and the local file is ignored); otherwise the Inno install marker written by `D06 T01 §3` means installed; otherwise the tool is portable and the local ini is created on first write. Done when: each rule is asserted on fixtures, including `Portable=0` beside the exe resolving installed, and a startup log line names the mode, the path, and the rule that fired.
- [ ] Fail loud when portable resolves in a directory the tool cannot write. Done when: startup reports the cause naming the directory, rather than falling back silently or dying on first write.
...
- [ ] Add assertions for round trip, defaults, atomic-write failure, migration, and the four mode-detection rules. Done when: eight assertions run.
...
**Test checkpoint:** `ctest` runs eight settings assertions green. An `.ini` written by the shipped AutoIt tool reads back identically. Mode detection is asserted per rule with the startup line quoted; portable in a read-only directory fails naming the directory. A write to a read-only target reports failure with the original intact. A fixture `.lng` migrates with its log line. All quoted.

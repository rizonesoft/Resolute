...
- [ ] Specify the Help menu whole: update check, publisher home, downloads, support, issue creation, and About (`D01 T01 §12`), each with its pack key and target; the Donate item carries the §7 verdict. Done when: every item names its target and the About entry opens the §12 dialog for the running tool's descriptor. Cheaper substitute that fails the checkpoint: a Help menu whose URLs live in each tool, which is how fourteen support links rot separately.

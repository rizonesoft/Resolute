...
After the plan review completes, with the findings file final (round records, plan review, and ledger all written; no finding edit follows), emit the review's attestation beside the findings file: one JSON file per review, naming the sign-off round's manifest, candidate, tree, reviewer, verdict, and checker result. Attesting earlier binds bytes the plan review then rewrites, and the stamp read-back fails the finalized file as a replacement (D00 T04 §21 review fix F2).

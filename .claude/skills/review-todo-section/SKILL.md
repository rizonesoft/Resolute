...
git --no-replace-objects diff --quiet || { echo "BLOCKED: unrelated worktree edits present; stash or commit them and restart"; exit 1; }  # whole tree clean: anchors read cited files from disk, so any unstaged edit anywhere voids the byte-equality the stamp set proves

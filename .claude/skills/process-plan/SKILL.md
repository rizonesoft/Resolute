...
Every way a run ends (closeout, park, plan done, operator stop) deletes all three: the guard file, the state file, and the heartbeat job.
...
1. If build/claude-campaign-guard.json is missing, the run is over: CronDelete this job (find it with CronList by this prompt's first sentence), delete build/claude-campaign-state.json if present, and reply RUN FINISHED.
2. If <run file> has a line "## Closeout" or a column-0 line starting "PARKED", run `python scripts/todo-graph.py query ready`. If it prints `0 runnable now`, the plan run is over: CronDelete this job, delete build/claude-campaign-guard.json and build/claude-campaign-state.json, and reply RUN FINISHED. Otherwise the phase ended but the plan did not: delete nothing, and resume under step 4, where process-plan re-points the guard to the next phase. A phase closeout alone never ends a plan run.
3. If build/claude-campaign-state.json has trips of 2 or more: the run stalled twice with no change to the tree. Do not resume. Append a Critical events line to the run file naming what blocks it, delete the guard file and the state file, CronDelete this job, and report the stall to the operator.
4. Otherwise resume the campaign under process-plan: re-run `python scripts/todo-graph.py query ready`, pick up the open section in the run file exactly where it stopped, and keep shipping: finish the section, stamp it, then the next section, then the next phase. A commit is not a stop, and a red CI is repaired, not waited on. Stop only at closeout, park, or an operator stop.
```

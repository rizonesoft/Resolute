...
1. Reconcile first: `CronList`, then `python scripts/campaign_guard.py reconcile --session $CLAUDE_CODE_SESSION_ID --jobs <every job id whose prompt contains Claude run-guard heartbeat for Resolute, comma-separated>`, and do what it prints: `CronDelete` an orphan or stale job, and start nothing when another session owns the guard. A crash between creating a job and acquiring a guard is repaired here, before a second one is made (D00 T04 §36).
...
```text
Claude run-guard heartbeat for Resolute Phase <N> (run file <run file>, generation <generation>). This session went idle while a campaign run may still be open. Check, then act, in this turn.

1. Run `python scripts/campaign_guard.py whoami --session $CLAUDE_CODE_SESSION_ID --generation <generation>`. If it prints NOT THE OWNER or NOT THE CURRENT JOB, this job is not the run's: CronDelete this job, read and change nothing else, and reply with that line. If it prints MALFORMED GUARD, run step 2, report the malformed guard to the operator, and stop.
...
3. Run `python scripts/campaign_guard.py pending-cancel`. If it names a job, CronDelete that job, confirm it gone with CronList, and run `python scripts/campaign_guard.py cancel-confirmed --cron-id <that job>`.
4. If step 1 printed NO GUARD, the run is over: run `python scripts/campaign_guard.py reset-state --expect-no-guard yes`, CronDelete this job (find it with CronList by this prompt's first sentence), and reply RUN FINISHED.
5. If <run file> has a line "## Closeout" or a column-0 line starting "PARKED" written after this run began, run `python scripts/todo-graph.py query ready`. If it prints `0 runnable now`, the plan run is over: run `python scripts/campaign_guard.py end --session $CLAUDE_CODE_SESSION_ID --reason plan-done`, CronDelete this job, run `cancel-confirmed` for it, and reply RUN FINISHED. Otherwise the phase ended but the plan did not: delete nothing, and resume under step 7, where process-plan re-points the guard to the next phase. A phase closeout alone never ends a plan run.
6. If build/claude-campaign-state.json has trips of 2 or more: the run stalled twice with no change to the tree. Do not resume. Append a Critical events line to the run file naming what blocks it, run `python scripts/campaign_guard.py end --session $CLAUDE_CODE_SESSION_ID --reason stall`, CronDelete this job, run `cancel-confirmed` for it, and report the stall to the operator.
7. Otherwise append `- bookkeeping: heartbeat resumed <UTC>` to the run file's Critical events and resume the campaign under process-plan: re-run `python scripts/todo-graph.py query ready`, pick up the open section in the run file exactly where it stopped, and keep shipping: finish the section, stamp it, then the next section, then the next phase. A commit is not a stop, and a red CI is repaired, not waited on. Stop only at closeout, park, or an operator stop.
```

...
1. If build/claude-campaign-guard.json is missing: the run is over. If <run file> has a line "## Closeout" or a column-0 line starting "PARKED", run `python scripts/todo-graph.py query ready`: when it prints runnable-now rows the phase ended but the plan did not, so resume process-plan, which re-points the guard to the next phase (step 3 below); only when it prints `0 runnable now` is the run over. CronDelete this job (find it with CronList by this prompt's first sentence), delete build/claude-campaign-guard.json and build/claude-campaign-state.json if present, and reply RUN FINISHED. A phase closeout alone is never the end of a plan run.

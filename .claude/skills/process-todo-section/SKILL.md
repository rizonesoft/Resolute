...
Push the SHIP push. The commit must exist on the remote before the next step, because the independent reviewer reads the commit. Capture the remote head first (`BEFORE=$(git ls-remote origin refs/heads/master | cut -f1)`) and read CI back after it with `python scripts/review_prompt.py ci-wait <SHIP_SHA> --since $BEFORE` (D00 T04 §30), and record its line in the run file: a red read-back is repaired before the review, never carried into it, by the bounded repair loop the review skill's push block states (D00 T04 §31), escalating only a cause the tree cannot fix. After a repair, `<SHIP_SHA>` is the repaired head: re-run the checkpoint there and give step 8 that sha, because the pre-repair commit is not the candidate any more.
...
**Corrected 2026-09-17: every scope flag refuses a prompt, and a BARE prompt works.** The conflict is not specific to `--commit`. `--base <BRANCH>` and `--uncommitted` refuse a prompt in exactly the same way, each naming its own flag. But `codex review "<instructions>"` with **no scope flag** is legal, reviews the latest commit when the working tree is clean, and **obeys the instructions**: driven with a prompt asking for error-handling defects only and a sentinel phrase, and the sentinel came back.

So an **instructed** pass is available and costs nothing to run. Its price is that the scope is implicit: a bare prompt cannot pin a SHA, so it is only trustworthy with a clean tree immediately after the SHIP push, when the latest commit is the one under review. Check the summary names the right work before believing it. The pinned `--commit` pass above stays the pass of record, because a stamp needs a scope that cannot be misread.

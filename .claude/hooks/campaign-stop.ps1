...
#     fingerprint, blocks without progress, stall trips.
...
try {
    ...
    $bytes = [System.Text.Encoding]::UTF8.GetBytes("$head`n$diff`n$untracked`n$text")
    ...
    $message += " Finished means a '## Closeout' heading or a column-0 'PARKED' line in the run file. If you are truly blocked on the operator, say what you need and end the turn again; the breaker allows the stop after $MaxBlocksWithoutProgress pushes with no tree change (this is push $($state.blocks))."
    $payload = @{ decision = "block"; reason = $message } | ConvertTo-Json -Compress
    [Console]::Out.WriteLine($payload)
    exit 0
}
catch {
    [Console]::Error.WriteLine("campaign-stop: $($_.Exception.Message)")
    exit 0
}
